...
{
    var ajaxurl = "get_code";
    $.ajax({
        url: ajaxurl,
        dataType: "json",
        success: function (ajaxobj) {
            if (ajaxobj.response_code) {
                if (func != null) {
                    func.call(this);
                }
                $.showSuccess(ajaxobj.show_err, function () {
                    to_send_msg = true;
                });
            }
            else {
                $("#reveiveActiveCode").removeClass("btn_disable");
                $.showErr(ajaxobj.show_err);
            }
        },
        error: function (ajaxobj)
        {
...
function collect_deal(deal_id)
{
    var attention = $('#get_attention').attr('data');
    var ajaxurl='';
    if (attention == 0) {
        ajaxurl= ROOT+MODULE_NAME+"/"+CONTROLLER_NAME+"/collect?deal_id="+deal_id;
    } else {
        ajaxurl= ROOT+MODULE_NAME+"/"+CONTROLLER_NAME+"/cancel?deal_id="+deal_id;
    }
    $.ajax({
        url: ajaxurl,
        dataType: "json",
        success: function (obj) {
            if (obj.status == -1)
            {
                $.weeboxs.open(obj.html, {contentType: 'text', showButton: false, title: "请先登录", width: 570, type: 'wee'});
            }
            else
            {
                if (attention == 1) {
                    $('#get_attention').attr('data', 0);
                    $('#get_attention').text('关注+');
                } else if (attention == 0) {
                    $('#get_attention').attr('data', 1);
                    $('#get_attention').text('取消关注');
                }
...
function fly_gotop() {
    $("#gotop").animate({bottom: "600px", opacity: "0"}, 1000, "linear", function () {
        $("#gotop").css("bottom", "10px");
    });
}
/*
...
function idcheck(o) {
    ...
    if (str.length == 15) {
        ...
        byear.val(19 + id[2]);
        bmonth.val(parseInt(id[3]));
        bday.val(parseInt(id[4]));
