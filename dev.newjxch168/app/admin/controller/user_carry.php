...
class UserCarry extends backend{
    //表名
    protected $tableName = 'user_carry';
    
    //所有用户提现列表
    public function index(){
        $carry_list = D('user_cash')->getListData($this->tableName,$_REQUEST);
        $this->assign("page", $carry_list['page']);
        $this->assign("nowPage", $carry_list["nowPage"]);
        $this->assign("list",$carry_list['data_list']);
        return $this->fetch();
    }
    ...
    public function statistics(){
        ...
        //获取表格数据
        $carry_account = D('user_cash')->getAccount($this->tableName,$date_list);
        $this->assign("list",$carry_account);
        return $this->fetch();
    }
    ...
    public function rank(){
        //获得表格数据
        $user_carry_rank = D('user_cash')->getUserRank($this->tableName,$_REQUEST);
        $this->assign("page", $user_carry_rank['page']);
        $this->assign("nowPage", $user_carry_rank["nowPage"]);
        $this->assign("list",$user_carry_rank['data_list']);
        return $this->fetch();
    }
}
