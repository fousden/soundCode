...
class Db {
    ...
    public static function instance($config = [], $lite = false) {
        ...
        if (!isset(self::$instances[$md5])) {
            ...
            $type = isset($options['database']['default']['type']) ? $options['database']['default']['type'] : $options['type'];
            // 如果采用lite方式 仅支持原生SQL 包括query和execute方法
            $class = $lite ? '\\think\\db\\Lite' : (!empty($options['namespace']) ? $options['namespace'] : '\\think\\db\\driver\\') . ucwords($type);
