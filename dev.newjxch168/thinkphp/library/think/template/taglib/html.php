...
class Html extends Taglib{
    ...
    public function _list($tag) {
        ...
        $style      = $tag['style'];                    //样式名
        $name       = !empty($tag['name'])?$tag['name']:'vo';                 //Vo对象名
        $action     = isset($tag['action'])&&$tag['action']=='true'?true:false;                   //是否显示功能操作
        $key         =  !empty($tag['key'])?true:false;
        $sort      =  isset($tag['sort'])&&$tag['sort']=='false'?false:true;
        $checkbox   = isset($tag['checkbox'])?$tag['checkbox']:false;                 //是否显示Checkbox
        if(isset($tag['actionlist'])) {
		...
		$parseStr	= "<!-- Think 系统列表组件开始 -->\n";
        $parseStr  .= '<table id="'.$id.'" class="'.$style.'" cellpadding=0 cellspacing=0 >';
        $parseStr  .= '<tr><td height="5" colspan="'.($colNum+1).'" class="topTd" ></td></tr>';
        $parseStr  .= '<tr class="row" >';
        //列表需要显示的字段
        $fields = array();
        foreach($show as $val) {
        ...
        if(!empty($key)) {
         ...
         //$class_key,$class_arr用于td的class值
        $class_key=array();
        $class_arr=array();
        foreach($fields as $k=>$field) {//显示指定的字段
            $property = explode('|',$field[0]);
            $showname = explode('|',$field[1]);
            if(isset($showname[1]) && stripos(trim($showname[1]),'class')!==false){
                $class_key[$k]=$k;
                $class_arr[$k]=trim($showname[1]);
            }else if(isset($showname[2]) && stripos(trim($showname[2]),'class')!==false){
                $class_key[$k]=$k;
                $class_arr[$k]=trim($showname[2]);
            }else if(isset($showname[3]) && stripos(trim($showname[3]),'class')!==false){
                $class_key[$k]=$k;
                $class_arr[$k]=trim($showname[3]);
            }
            ...
            if($sort) {
                $parseStr .= '<a href="javascript:sortBy(\''.$property[0].'\',\'{$sort}\',\''.ACTION_NAME.'\')" title="按照'.$showname[2].'{$sortType} ">'.$showname[0].'<eq name="order" value="'.$property[0].'" ><img src="__ADMIN__/images/{$sortImg}.gif" width="12" height="17" border="0" align="absmiddle"></eq></a></th>';
            }else{
                $parseStr .= $showname[0].'</th>';
            }
        ...
        foreach($fields as $k=>$field) {
            if(in_array($k,$class_key)){
                $parseStr .= '<td '.$class_arr[$k].'>&nbsp;';
            }else{
                $parseStr .= '<td>&nbsp;';
            }
            //显示定义的列表字段
//            $parseStr   .=  '<td>';
        ...
        $parseStr	.= '</tr></volist><tr><td height="5" colspan="'.($colNum+1).'" class="bottomTd"></td></tr></table>';
        $parseStr	.= "\n<!-- Think 系统列表组件结束 -->\n";
        return $parseStr;
    }
}