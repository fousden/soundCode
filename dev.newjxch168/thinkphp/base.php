...
defined('COMMON_MODULE') or define('COMMON_MODULE', 'base');
 ...
 * @return \Think\Model
 */
function M($name = '', $dbName= 'default')// , $tablePrefix = '', $connection = ''
{
    $config = \think\Config::get('database.database');
    $row    = $config[$dbName];
    return \think\Loader::table($name, ['prefix' => $row['prefix'], 'connection' => $row]);
}
 ...
 * @return object
 */
function D($name = '', $dbName= 'default',$layer = MODEL_LAYER)
{
    $config = \think\Config::get('database.database');
    return \think\Loader::model($name, $layer,$config[$dbName]);
}
