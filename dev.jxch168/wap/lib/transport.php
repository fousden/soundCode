...
class transport
{
    ...
    function request($url, $params = '', $method = 'POST', $my_header = '')
    {
        ...
        if (isset($_GET['dg']) && isset($_GET['dg']) == 'jxch168')
        {
            file_put_contents(APP_ROOT_PATH . 'log/wap2mapi/' . date('Y-m-d') . '_wap2mapi.log', "URL:[" . $url. "];method[ " . $method . "];param:[" . json_encode($params) . "];response[" .json_encode($response). "][" . date('Y-m-d H:i:s') . "]\r\n", FILE_APPEND);
        }
