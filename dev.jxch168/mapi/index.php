...
// +----------------------------------------------------------------------
// | jxch168 金享财行 系统 mapi 插件
// +----------------------------------------------------------------------
// | Copyright (c) 2010 http://www.jxch168.com/ All rights reserved.
// +----------------------------------------------------------------------
...
$func    = strtolower(strim($request['func'])) ? strtolower(strim($request['func'])) : "index";
$city_id = intval($request['city_id']);

if ($class == 'register_red')
{
    $class ="register";
}
...
$user_info = user_check($GLOBALS['request']['email'],$GLOBALS['request']['pwd'],false);
