...
class init{
	public function index()
	{
		...
		if(true || $index_list===false)
		{
			...
			$limit = "0,5";
			$orderby = "deal_status asc,rate desc,update_time desc ";
			...
			foreach ($vdata as $value) {
				if($user_id > 0){
					$has_bid_count_sql = "SELECT count(1) FROM ".DB_PREFIX."deal_load WHERE deal_id=".$value["id"]." AND user_id=".$user_id;
					logger::write($has_bid_count_sql);
					$has_bid_count = $GLOBALS['db']->getOne($has_bid_count_sql);
					if($has_bid_count == 0 && $value["cate_id"] == "5"){
						array_push($rdata,$value);
					}else{
						array_push($rdata,$value);
					}

				}else if($value["cate_id"] == "5"){
					array_push($rdata,$value);
				}
			}*/
			...
			foreach($result['list'] as $value){
				...
				//不显示流标和待审核的标
				else if($value['deal_status'] == 3 || $value['deal_status'] == 0 || $value['deal_status'] == 5)
				{
				}
				else{
					array_push($rdata, $value);
