...
class register {
	...
	$referer = strim($GLOBALS['request']['referer']);
        $search_channel=$GLOBALS['request']['search_channel'];

        if ($search_channel!='jbh2015' && !$mobile_code)
        {
                output("请输入手机验证码");
        }
	$mobileId = strtoupper(strim($GLOBALS['request']['jxstr']));
	$channel = strim($GLOBALS['request']['channel']);

	$active = $GLOBALS['request']['active'];
	$bonusId = $GLOBALS['request']['bonusId'];
	...
	if (isset($GLOBALS['request']['r'])) {
	    $is_sale_invite = mb_substr($GLOBALS['request']['r'], 0, 4);
	    if ($is_sale_invite == 'jxch') {
		$referer = $GLOBALS['request']['r'];
	    } else {
		$referer = strim(base64_decode($GLOBALS['request']['r']));
	    }
	}
	...
	$root['referer_base'] = $referer;
	$root['active'] = $active;
	$root['bonusId'] = $bonusId;
	...
	if (isset($user_data['referer']) && $user_data['referer'] != "") {
	    ...
	    if (!$p_user_data) {
		$is_sale_invite = $GLOBALS['request']['referer'];
	    ...
	    } else if ($p_user_data["user_type"] == 3) {
	...
	if ($check_status['status'] == 1) {
	    ...
	    //短信验证码
	    if ($mobile_code != $GLOBALS['db']->getOne("select verify_code from " . DB_PREFIX . "mobile_verify_code where mobile = '" . $mobile . "' and create_time>=" . (TIME_UTC - 300) . " ORDER BY id DESC")) {
		$root['response_code'] = 0;
		$root['show_err'] = "短信验证码错误或已失效";
		output($root);
	    } else {
		require_once APP_ROOT_PATH . "system/libs/user.php";
		if ($user_data['terminal'] == 4 || $user_data['terminal'] == 3) { //Android IOS 渠道信息更新
		    $extensionSource = $GLOBALS['db']->getOne("select source from " . DB_PREFIX . "mobile_extension where upper(udid) = upper('" . $mobileId . "') and state = 1 order by test_time asc");
		    if ($extensionSource) {
			$user_data['search_channel'] = $extensionSource;
			if ($user_data['terminal'] == 3) {
			    $user_data['search_channel'] = $channel;
			}

			$sql_extension = "update " . DB_PREFIX . "mobile_extension set state = 2,test_time = " . time() . " where upper(udid) = upper('" . $mobileId . "') and source ='{$user_data['search_channel']}'";
			$GLOBALS['db']->query($sql_extension);
		    } else if ($user_data['terminal'] == 4) { //IOS处理
			$user_data['search_channel'] = "app_store";
		    }
		}

		if (empty($user_data['search_channel'])) {
		    $user_data['search_channel'] = $GLOBALS['request']['search_channel'];
		}
		...
		if ($user_id) {
			...
			$root['user_pwd_md5'] = md5($user_data['user_pwd'].date("Y-m-d"));
			...
			if ($active && $bonusId) {
			    //活动信息
			    send_bonus($user_data, $reg_time, $active, $bonusId);
			}
			MO('BonusMiddel')->middel2formal(array('id' => $user_data['id'], 'user_name' => $user_data['user_name'], 'mobile' => $user_data['mobile']));
			...
			MO("Channel")->updateChannel($user_data['mobile'],$user_id);
                        //百度渠道注册后送100元优惠劵
                        if($user_data['search_channel']=='baidu'){
                            MO("ActivityConf")->GlobalActivity(2,$user_id);
                        }
                        //友盟推送
	...
	$user['create_date'] = date("Y-m-d", TIME_UTC);
	$user['pid'] = (int) $user_data['pid'];
	$user['referer_memo'] = $user_data['referer_memo'];

	$user['mobile_id'] = $user_data['mobile_id'];
	$user['terminal'] = $user_data['terminal'];
	$user['search_channel'] = $user_data['search_channel'];
	$user['admin_id'] = (int) $user_data['admin_id'];
	...
	$user['mobile'] = $user_data['mobile'];
        //富友开户时用到的账户名 默认为用户手机号码
	$user['fuiou_account'] = $user_data['fuiou_account'];
