...
class loantype_2 implements loantype{
	...
	function deal_repay_money($deal){
		...
		if($deal['repay_time_type'] == 0){
			$deal['rate'] = $deal['rate']/30;
                ...
                //去除自动投标的金额
                $auto_deal_loads = $GLOBALS['db']->getOne("select sum(money) from " . DB_PREFIX . "deal_load where deal_id = " . $deal['id'] . " and is_auto = 1");
                $deal['borrow_amount'] = $deal['borrow_amount'] - $auto_deal_loads;
		//月还本息
	...
	function make_repay_plan($deal){
		...
		for($i=0;$i<$true_repay_time;$i++){
			$load_repay = array();
			if($deal['repay_time_type']==0){
				$repay_day = $load_repay['repay_time'] = $repay_day + $deal['enddate']*24*3600 + 24*3600 + $deal['repay_time']*24*3600;
                        }else{
			...
			$load_repay['manage_money_rebate'] = 0;

			$load_repay['deal_id'] = $deal['id'];
			$load_repay['user_id'] = $deal['user_id'];

			//$load_repay['repay_money'] = $load_repay['repay_money'] ;
	...
	function make_user_repay_plan($deal,$idx,$repay_day,$true_time,$repay_id,$load_users,&$total_money){
		...
		foreach($load_users as $k=>$v){
			...
			if($idx+1 == $true_repay_time){
				$item['repay_money'] = $item['money'] + ($item['money']*$deal['rate']/12/100*$all_repay_time)* $deal['yield_ratio'];
...
//			$repay_data['repay_date'] = to_date($repay_day);
			$repay_data['repay_time'] = strtotime($deal['last_mback_time']);
			$repay_data['repay_date'] = $deal['last_mback_time'];
                        $repay_data['self_money'] = num_format($vv['money']);
                        $repay_data['true_self_money'] = num_format($vv['money']- $vv['coupon_cash']);
                        //纯收益
			$repay_data['pure_interests'] = num_format($vv['pure_interests']);
                        //活动收益
                        $repay_data['active_interest_money'] = num_format($vv['act_interests']);
                        //收益券收益
                        $repay_data['coupon_interests'] = num_format($vv['coupon_interests']);
                        //总收益
			$repay_data['interest_money'] = num_format($vv['pure_interests'] + $vv['act_interests'] + $vv['coupon_interests']);
			$repay_data['true_interest_money'] = num_format($repay_data['interest_money']);

                        /*************活动一 中秋提“钱”过 活动*************/
                        /*$mid_autumn_act = C('mid_autumn_acting');
                        $active_interest_money = get_act_income($mid_autumn_act,$repay_data['interest_money'],$vv['create_time']);
                        if($active_interest_money){
                            $repay_data['active_interest_money'] = num_format($active_interest_money);
                            $repay_data['interest_money'] = $repay_data['interest_money'] + $repay_data['active_interest_money'];
                        }*/
                        /************* 优惠券相关收益 *************/
                        /*$deal_load_infos = $GLOBALS['db']->getRow("select coupon_id,coupon_interests from ".DB_PREFIX."deal_load where id = '".$repay_data['load_id']."'");
                        ...
                        if($coupon_type == 1 && $deal_load_infos['coupon_interests'] > 0){
                            $repay_data['coupon_interests'] = num_format($deal_load_infos['coupon_interests']);
                            $repay_data['interest_money'] = $repay_data['interest_money'] + $repay_data['coupon_interests'];
                        }*/
			//本息总额
			$repay_data['repay_money'] = $repay_data['self_money'] + $repay_data['interest_money'];
                        $repay_data['true_repay_money'] = num_format(@$repay_data['repay_money']);
                        //$repay_data['repay_money'] = num_format($repay_data['repay_money']) + num_format($repay_data['interest_money']);
			...
			$repay_data['manage_interest_money'] = 0;
			...
			$repay_data['manage_interest_money_rebate'] = 0;
