...
function save_user($user_data, $mode = 'INSERT') {
    ...
    if ((intval($_REQUEST["REGISTER_TYPE"]) == 0 || intval($_REQUEST["REGISTER_TYPE"]) == 1) && intval($user_data["user_type"]) != 2 && intval($user_data["user_type"]) != 3 && (($mode == "INSERT" && (intval(app_conf('REGISTER_TYPE')) == 0 || intval(app_conf('REGISTER_TYPE')) == 1 ) ) || isset($user_data['mobile']))) {
        ...
        //手机禁止号码更改
        $user_mobile_info = $GLOBALS['db']->getRow("select mobile,mobilepassed from " . DB_PREFIX . "user where id = " . intval($user_data['id']));
        if ($user_mobile_info['mobile'] && $user_mobile_info['mobilepassed'] == 1 && $user_data['mobile'] != $user_mobile_info['mobile']) {
            $field_item['field_name'] = 'mobile';
            $field_item['error'] = NOT_EDIT_ERROR;
            $res['status'] = 0;
            $res['data'] = $field_item;
            return $res;
        }
    ...
    //担保方图片添加，如果没有则插入，如果有则更新
    if (isset($user_data['is_db_img']))
        $user['is_db_img'] = $user_data['is_db_img'];
...
//		$user['vip_state'] = 1;
    ...
    if ($mode == 'INSERT' && isset($user_data['fuiou_account']) && $user_data["fuiou_account"] != "") {
...
function do_login_user($user_name_or_email, $user_pwd) {
    ...
    if (!$user_data) {
        ...
        if (strlen($user_pwd) == 32 && $user_data['user_pwd'] == $user_pwd) {
            ...
            $is_use_pass = true;
        } else if (md5($user_data['user_pwd'] . date("Y-m-d")) == $user_pwd) {
            $is_use_pass = true;
        } else if (md5($user_data['user_pwd'] . date("Y-m-d", strtotime("+1 year"))) == $user_pwd) {
            $is_use_pass = true;
        } else if (md5($user_data['user_pwd'] . date("Y-m-d", strtotime("-1 day"))) == $user_pwd) {
            $is_use_pass = true;
        }
...
//					}
                ...
                $GLOBALS['db']->query("update " . DB_PREFIX . "user set login_ip = '" . CLIENT_IP . "',login_time= " . TIME_UTC . ",group_id=" . intval($user_data['group_id']) . " where id =" . $user_data['id']);
                $login_log['user_name'] = $user_data['user_name'];
                $login_log['login_time'] = time();
                $login_log['terminal'] = getTerminalId(strim($GLOBALS['request']['_m']));
                $login_log['ip_address'] = get_client_ip();
                $GLOBALS['db']->autoExecute(DB_PREFIX . "user_login_log", $login_log, "INSERT");
                //友盟推送
                $device_data['user_id'] = $user_data['id'];
                $device_data['device_token'] = trim($_REQUEST['device_token']);
                $device_data['device_type'] = trim($_REQUEST['_m']);
                MO("devicePush")->add($device_data);
                if ($GLOBALS['request']['openid']) {
                    $res = MO("WeixinUser")->updateOpenidBy($user_data['id'], $GLOBALS['request']['openid']);
                }
