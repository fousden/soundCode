...
require_once 'extend_common.php';
...
function get_gmtime() {
    ...
    if ($now_time_stamp >= $fix_time_stamp) {
        ...
        $return_time_stamp = time();
    } else {
        $return_time_stamp = (time() - date('Z'));
    }
    return $return_time_stamp;
}
...
function php_export_excel($objPHPExcel, $filename = "Excel") {
    ...
    $objPHPExcel->getActiveSheet()->getStyle('A1:Z1')->applyFromArray(array('font' => array('bold' => true), 'alignment' => array('horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER)));
    $objPHPExcel->getActiveSheet()->getStyle('A2:Q2000')->applyFromArray(array('alignment' => array('horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER)));
    //设置行高
...
function syn_dealing() {
    $deals = $GLOBALS['db']->getAll("select id from " . DB_PREFIX . "deal where is_effect = 1 and deal_status not in (2,3,4,5) and is_delete = 0 AND load_money/borrow_amount <= 1");
    foreach ($deals as $v) {
...
function syn_deal_status($id) {
    ...
    if ($deal_info['deal_status'] != 3) {
        if ($deal_info['progress_point'] < 100) {
            ...
            $data['progress_point'] = $deal_info['progress_point'] = num_format($data['load_money'] / $deal_info['borrow_amount'] * 100, 2);
        }
        ...
        if (($deal_info['progress_point'] >= 100 || $data['progress_point'] >= 100) && floatval($deal_info['load_money']) >= floatval($deal_info['borrow_amount'])) {
            ...
            elseif (($deal_info['deal_status'] == 4 && $deal_info['repay_start_time'] > 0) || ($deal_info['deal_status'] == 2 && $deal_info['repay_start_time'] > 0 && $deal_info['repay_start_time'] <= $deals_time)) {
                ...
                if ($deal_info['success_time'] == 0) {
                    $deal_info['success_time'] = $GLOBALS['db']->getOne("SELECT max(create_time) FROM " . DB_PREFIX . "deal_load WHERE deal_id=$id ");
                }
                //更新标的起息时间 结息时间
                $data['success_time'] = $deal_info['success_time'];
                $data['qixi_time'] = date("Y-m-d", strtotime("+1 day", $data['success_time']));
                $data['jiexi_time'] = date("Y-m-d", strtotime("+" . $deal_info['repay_time'] . " days", strtotime($data['qixi_time'])));
                $data['last_mback_time'] = date("Y-m-d", strtotime("+2 days", strtotime($data['jiexi_time']))); //默认是结息日后3天
...
function make_repay_plan($deal) {
    ...
    foreach ($list as $i => $load_repay) {
        $load_repay['manage_impose_money'] = num_format($load_repay['manage_impose_money']);
        $load_repay['repay_money'] = num_format($load_repay['repay_money']);
        $load_repay['manage_money'] = num_format($load_repay['manage_money']);
        $load_repay['impose_money'] = num_format($load_repay['impose_money']);
        $load_repay['true_self_money'] = num_format($load_repay['true_self_money']);
        $load_repay['true_repay_money'] = num_format($load_repay['true_repay_money']);
        $load_repay['true_self_money'] = num_format($load_repay['true_self_money']);
        $load_repay['true_interest_money'] = num_format($load_repay['true_interest_money']);
        $load_repay['interest_money'] = num_format($load_repay['interest_money']);
        $load_repay['true_manage_money'] = num_format($load_repay['true_manage_money']);
        $load_repay['self_money'] = num_format($load_repay['self_money']);
        $load_repay['manage_money_rebate'] = num_format($load_repay['manage_money_rebate']);
        $load_repay['true_manage_money_rebate'] = num_format($load_repay['true_manage_money_rebate']);
...
function make_user_repay_plan($deal, $idx, $repay_day, $true_time, $repay_id, &$total_money) {
    ...
    if (!isset($fload_users[$deal['id']])) {
        $fload_users[$deal['id']] = $GLOBALS['db']->getAll("SELECT * FROM " . DB_PREFIX . "deal_load  WHERE deal_id=" . $deal['id'] . " AND is_auto = 0 AND contract_no != '' ORDER BY id ASC ");
    }
    ...
    foreach ($load_users as $kk => $vv) {
        ...
        $repay_data['self_money'] = num_format(@$repay_data['self_money']);
        $repay_data['repay_money'] = num_format(@$repay_data['repay_money']);
        $repay_data['manage_money'] = num_format(@$repay_data['manage_money']);
        $repay_data['impose_money'] = num_format(@$repay_data['impose_money']);
        $repay_data['true_repay_money'] = num_format(@$repay_data['true_repay_money']);
        $repay_data['true_self_money'] = num_format(@$repay_data['true_self_money']);
        $repay_data['interest_money'] = num_format(@$repay_data['interest_money']);
        $repay_data['true_interest_money'] = num_format($repay_data['interest_money']);
        //活动收益
        $repay_data['active_interest_money'] = num_format(@$repay_data['active_interest_money']);
        //优惠券收益
        $repay_data['coupon_interests'] = num_format(@$repay_data['coupon_interests']);
        //纯利息
        $repay_data['pure_interests'] = num_format($vv['pure_interests']);

        $repay_data['true_interest_money'] = num_format(@$repay_data['true_interest_money']);
        $repay_data['true_manage_money'] = num_format(@$repay_data['true_manage_money']);
        $repay_data['true_repay_manage_money'] = num_format(@$repay_data['true_repay_manage_money']);

        $repay_data['repay_manage_money'] = num_format(@$repay_data['repay_manage_money']);
        $repay_data['repay_manage_impose_money'] = num_format(@$repay_data['repay_manage_impose_money']);
        $repay_data['true_manage_interest_money'] = num_format(@$repay_data['true_manage_interest_money']);
        $repay_data['manage_interest_money_rebate'] = num_format(@$repay_data['manage_interest_money_rebate']);
        $repay_data['true_manage_interest_money_rebate'] = num_format(@$repay_data['true_manage_interest_money_rebate']);
        $repay_data['reward_money'] = num_format(@$repay_data['reward_money']);
        $repay_data['true_reward_money'] = num_format(@$repay_data['true_reward_money']);
        ...
        if ($old_info = $GLOBALS['db']->getRow("SELECT id,has_repay FROM " . DB_PREFIX . "deal_load_repay WHERE deal_id=" . $vv['deal_id'] . " AND u_key=$kk and l_key= $idx ")) {
            if ($old_info['has_repay'] == 1) {
                ...
                unset($repay_data['active_interest_money']);
    ...
    $all_money = $GLOBALS['db']->getRow("SELECT sum(repay_money) as total_repay_money,sum(interest_money) as total_interest_money FROM " . DB_PREFIX . "deal_load_repay WHERE deal_id=" . $vv['deal_id'] . " AND l_key= $idx ");
    $total_repay_money = num_format($all_money['total_repay_money']);
    $total_interest_money = num_format($all_money['total_interest_money']);
    if ($all_money) {
        $GLOBALS['db']->query("UPDATE " . DB_PREFIX . "deal_repay SET repay_money = '" . $total_repay_money . "',interest_money = '" . $total_interest_money . "' WHERE deal_id=" . $vv['deal_id'] . " AND l_key= $idx");
    }
}
...
function sys_user_status($user_id, $is_cache = false, $make_cache = false) {
    ...
    if ($data == false) {
        ...
        $load_info = $GLOBALS['db']->getRow("SELECT sum(interest_money) as load_earnings,sum(repay_money) AS load_repay_money,sum(true_reward_money) as load_reward_money FROM " . DB_PREFIX . "deal_load_repay  WHERE  ((user_id = " . $user_id . " and t_user_id = 0) or t_user_id = " . $user_id . ") AND has_repay=1 ");
        //已赚利息
...
//	}
    //获取投标列表
    $load_user_list = $GLOBALS['db']->getAll("SELECT user_name,user_id,create_time FROM " . DB_PREFIX . "deal_load WHERE is_auto = 0 AND deal_id=" . $deal_info['id']);
    if ($load_user_list) {
...
function send_verify_sms($mobile, $code, $user_info, $immediately, $is_limit = false) {
    $re = array('msg_id' => 0, 'status' => 0, 'msg' => '');
    if ($is_limit == true) {
    ...
    if (app_conf("SMS_ON") == 1) {
        $tmpl = $GLOBALS['db']->getRow("select * from " . DB_PREFIX . "msg_template where name = 'TPL_SMS_SAVE_PWD_VERIFY_CODE'");
        $tmpl_content = $tmpl['content'];
...
//【$msg_item['dest'] = "13122556699,15877996644,...."短信发送号码，可以一个可以多个，多个用逗号隔开】
//【$msg_item['content']短信内容】 【$handleType 5代表短信发送】 【$sendType发送接口选择 YY一信通 EN泉龙达】
function send_sms_email($msg_item, $handleType = 5, $sendType = "") {
    ...
    if ($msg_item['send_type'] == 0) {
        ...
        $result = $sms->sendSms($msg_item['dest'], $msg_item['content'], $handleType, $sendType);
        //发送结束，更新当前消息状态
...
//充值URL跳转
function incharge_skip_url($url, $parameter = array()) {
    $ch = curl_init();
    ...
    curl_setopt($ch, CURLOPT_NOBODY, 0); //设置是否返回body信息，1 不返回，0 返回
    //设置用户浏览器信息
    curl_setopt($ch, CURLOPT_USERAGENT, 'Mozilla/5.0 (Windows NT 6.1; WOW64; rv:39.0) Gecko/20100101 Firefox/39.0');
    curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1); //设置是否允许页面跳转 1 跳转，0 不跳转
    //执行
    $re = curl_exec($ch);
    if ($re === false) { //检错
...
function send_users_msg($title, $content, $from_user_id, $to_user_id, $create_time, $sys_msg_id = 0, $only_send = false, $is_notice = false, $fav_id = 0)
{
...
function do_loans($id, $repay_start_time, $type = 0) {
    ...
    //满标放款转账操作
    $flags = true;
    $load_list = $GLOBALS['db']->getAll("SELECT id,user_id,user_name,`money`,coupon_cash,`is_old_loan`,`rebate_money`,`bid_score`,`is_winning`,`income_type`,`income_value`,`contract_no`,`contract_no_flag` FROM " . DB_PREFIX . "deal_load where deal_id=" . $id . " and is_auto = 0 and is_rebate = 0 AND is_has_loans = 0");
    foreach ($load_list as $lk => $lv) {
        if ($lv['contract_no'] && 0 == $lv['contract_no_flag']) {
            ...
            //$arr = $fuyou->transferBmuAction($user_info['mobile'], $lv['money'], $lv['id'], $lv['contract_no']);
            //$arr = $fuyou->transferBmuAction($user_info['fuiou_account'], $lv['money'], $lv['id'], $lv['contract_no']);//满标放款 使用富友账号
            ...
            if ('0000' == $arr->plain->resp_code) {
                ...
                if ($type == 0) {
                    ...
                    if ($GLOBALS['db']->affected_rows()) {
                        if ($lv['is_old_loan'] == 0) {
                            ...
                            file_put_contents(APP_ROOT_PATH . 'log/fuyou/' . date('Y-m-d') . '_transferBmu.log', "[满标放款富友转账成功，成功的deal_load投标记录的ID为" . $lv['id'] . "，投标人ID为" . $lv['user_id'] . ",投标人用户名为" . $lv['user_name'] . ",成功金额为" . $lv['money'] . "，抵现券面值" . $lv['coupon_cash'] . "];return[返回信息" . json_encode($arr) . "];[" . date('Y-m-d H:i:s') . "]\r\n", FILE_APPEND);
                        }
                    }
                }
                if (!$tmp) {
                ...
                $flags = false;
                $fail_money += $lv['money'];
                file_put_contents(APP_ROOT_PATH . 'log/fuyou/' . date('Y-m-d') . '_transferBmu.log', "[满标放款富友转账失败，失败的deal_load投标记录的ID为" . $lv['id'] . "，投标人ID为" . $lv['user_id'] . ",投标人用户名为" . $lv['user_name'] . ",失败金额为" . $lv['money'] . "];return[返回信息" . json_encode($arr) . "];[" . date('Y-m-d H:i:s') . "]\r\n", FILE_APPEND);
            }
        }
        if ($flags) {
            //返利给用户
    ...
    if ($GLOBALS['db']->affected_rows() > 0) {
        ...
        if ($fail_money > 0) {
            $return['status'] = 1;
            $return['info'] = "部分放款成功，未成功放款投标请查看失败日志信息！";
            $fail_money = 0;
        } else {
            $return['status'] = 1;
            $return['info'] = "放款成功";
...
function isMobile() {
    ...
    $_SERVER['HTTP_X_WAP_PROFILE'] = empty($_SERVER['HTTP_X_WAP_PROFILE']) ? $GLOBALS['request']['HTTP_X_WAP_PROFILE'] : $_SERVER['HTTP_X_WAP_PROFILE'];
    $_SERVER['HTTP_VIA'] = empty($_SERVER['HTTP_VIA']) ? $GLOBALS['request']['HTTP_VIA'] : $_SERVER['HTTP_VIA'];
    $_SERVER['HTTP_ACCEPT'] = empty($_SERVER['HTTP_ACCEPT']) ? $GLOBALS['request']['HTTP_ACCEPT'] : $_SERVER['HTTP_ACCEPT'];
    $_SERVER['HTTP_USER_AGENT'] = empty($_SERVER['HTTP_USER_AGENT']) ? $GLOBALS['request']['HTTP_USER_AGENT'] : $_SERVER['HTTP_USER_AGENT'];
    ...
    if (isset($_SERVER['HTTP_USER_AGENT'])) {
        ...
        str_replace('Mac OS X', '', $_SERVER['HTTP_USER_AGENT'], $macCnt);
        str_replace('_', '', $_SERVER['HTTP_USER_AGENT'], $cnt); //var_dump($cnt);exit;
        if ($cnt >= 2 && $macCnt < 1) {
            return true;
        }
    }
...
//验证用户相关信息
function check_user_info($user_info, $ajax) {
    ...
    if ($verify_status['idcardpassed'] != 1 || !$verify_status['real_name']) {
        showErr("<div style='font-size:18px;color:#C40000'>您尚未实名认证！</div>请先完成实名认证再进行交易。", $ajax, url("index", "uc_account#security"));
        die();
    }
    //手机认证认证未通过，请完成手机认证再充值
    if ($verify_status['mobilepassed'] != 1) {
        showErr("<div style='font-size:18px;color:#C40000'>您尚未手机认证！</div>请先完成手机认证再进行交易。", $ajax, url("index", "uc_account#security"));
        die();
    }
    ...
    if ($bank_num['num'] != 1) {
        showErr("<div style='font-size:16px;color:#C40000'>您尚未绑定银行卡！</div>请先绑定银行卡再进行交易。", $ajax, url("index", "uc_money#bank"));
        die();
    }
    //支付密码是否设置
    if (!$verify_status['paypassword']) {
        showErr("<div style='font-size:16px;color:#C40000'>您尚未设置支付密码！</div>请先设置支付密码再进行交易。", $ajax, url("index", "uc_account#security"));
        die();
    }
}

//分享代码
function share_code($size = 0) {
    $str = '<style type="text/css">#share a img{width:24px;height:24px;margin-right:3px;}</style>';
    $str .= '<a title="分享到新浪微博"  onclick="shareTo(' . "'sinaminiblog'" . ')" href="javascript:void(0);" ><img src="/front/images/share/sinaminiblog.png "/></a>';
    $str .= '<a title="分享到腾讯微博"  onclick="shareTo(' . "'qqmb'" . ')" href="javascript:void(0);" ><img src="/front/images/share/qqmb.png "/></a>';
    $str .= '<a title="分享到微信"  onclick="shareTo(' . "'weixin'" . ')" href="javascript:void(0);" ><img src="/front/images/share/weixin.png "/></a>';
    $str .= '<a title="分享到QQ空间"  onclick="shareTo(' . "'qzone'" . ')" href="javascript:void(0);" ><img src="/front/images/share/qzone.png "/></a>';
    $str .= '<a title="分享到人人网"  onclick="shareTo(' . "'renren'" . ')" href="javascript:void(0);" ><img src="/front/images/share/renren.png "/></a>';
    $str .= '<a title="分享到开心网"  onclick="shareTo(' . "'kaixin001'" . ')" href="javascript:void(0);" ><img src="/front/images/share/kaixin001.png "/></a>';

    if (!$size) {
        $str .= '<a title="分享到QQ好友"  onclick="shareTo(' . "'qqim'" . ')" href="javascript:void(0);" ><img src="/front/images/share/qqim.png "/></a>';
        $str .= '<a title="分享到若邻网"  onclick="shareTo(' . "'wealink'" . ')" href="javascript:void(0);" ><img src="/front/images/share/wealink.png "/></a>';
        $str .= '<a title="分享到朋友网"  onclick="shareTo(' . "'qqxiaoyou'" . ')" href="javascript:void(0);" ><img src="/front/images/share/qqxiaoyou.png "/></a>';
        $str .= '<a title="分享到网易微博"  onclick="shareTo(' . "'neteasemb'" . ')" href="javascript:void(0);" ><img src="/front/images/share/neteasemb.png "/></a>';
        $str .= '<a title="分享到堆糖"  onclick="shareTo(' . "'duitang'" . ')" href="javascript:void(0);" ><img src="/front/images/share/duitang.png "/></a>';
        $str .= '<a title="分享到东方微博"  onclick="shareTo(' . "'eastdaymb'" . ')" href="javascript:void(0);" ><img src="/front/images/share/eastdaymb.png "/></a>';
        $str .= '<a title="分享到豆瓣9点"  onclick="shareTo(' . "'9dian'" . ')" href="javascript:void(0);" ><img src="/front/images/share/9dian.png "/></a>';
        $str .= '<a title="分享到百度贴吧"  onclick="shareTo(' . "'itieba'" . ')" href="javascript:void(0);" ><img src="/front/images/share/itieba.png "/></a>';
        $str .= '<a title="分享到139邮箱"  onclick="shareTo(' . "'139mail'" . ')" href="javascript:void(0);" ><img src="/front/images/share/139mail.png "/></a>';
    }
...
/**
 * 数字转换，如果数组超过一万，则返回带万的，如果少于万的就格式化
 * @param $num
 */
function number_format_myriad($num) {
    if ($num >= 10000) {
        $n = (int) ($num / 100) / 100;
        $arr = explode('.', $n);
        $int = $arr[0];
        ...
        if ($float == 0) {
            $float = '';
            return number_format($int) . '万';
        } else {
            return number_format($int) . '.' . $float . '万';
        }
    } else {
        $n = (int) ($num * 100) / 100;
        $arr = explode('.', $n);
...
//发送消息入库
function send_msg_to_db($mobile, $msg, $title = '') {
    $msg_data['dest'] = $mobile;
    $msg_data['send_type'] = 0;
    $msg_data['title'] = addslashes($title);
 ...
 */
function MO($name) {
    $file = APP_ROOT_PATH . 'app/Model/' . $name . 'Model.class.php';
    if (!file_exists($file)) {
        die($name . ' is not define');
    }
    require_once APP_ROOT_PATH . 'app/Model/BaseModel.class.php';
    require_once $file;
