...
function showErr($msg, $ajax = 0, $jump = '', $stay = 0)
{
    if ($ajax == 1) {
        ...
        if (!$jump && $jump == '')
            $jump = APP_ROOT . "/";
        $GLOBALS['tmpl']->assign('page_title', '错误提示页');
...
//显示成功
function showSuccess($msg, $ajax = 0, $jump = '',$stay = 0,$checked = 0)
{
    if ($ajax == 1) {
        $result['status'] = 1;
        $result['info']   = $msg;
...
function get_spec_gif_anmation($url, $width, $height)
{
    ...
    $gif_maker = new GIFEncoder(
            $frames, $framed, 0, 2, 0, 0, 0, "bin"   //bin为二进制   url为地址
    );
...
//获取指定新闻模块内容
function get_notice_type($limit = 0, $cate_id = "")
{
    if ($limit == 0)
        $limit = app_conf("INDEX_NOTICE_COUNT");
    if ($limit > 0) {
    ...
    $list = $GLOBALS['db']->getAll("select a.*,ac.type_id from " . DB_PREFIX . "article a left join " . DB_PREFIX . "article_cate ac on a.cate_id = ac.id where a.cate_id = " . $cate_id . " AND ac.type_id = 0 and ac.is_effect = 1 and ac.is_delete = 0 and a.is_effect = 1 and a.is_delete = 0 order by a.sort desc " . $limit_str);
...
function setSearchchannelCookie(){
    if ($_GET['s']) {
