...
class SiteBaseModule
{
    ...
    public function __construct()
    {
        ...
        $GLOBALS['tmpl']->assign("is_http", $is_http);
        ...
        if(es_session::get("user_info")){
            ...
            $sql='select count(*) from '.DB_PREFIX.'msg_box where to_user_id='.$user_info['id'].' and is_read=0 and is_delete=0';
            $user_msg_count=$GLOBALS['db']->getOne($sql);
            $GLOBALS['tmpl']->assign("user_msg_count", $user_msg_count);
        }
    ...
    {
        $tmpStrat = explode(" ", $this->start_time);
        $TmpNow   = explode(" ", microtime());

        var_dump($tmpStrat[0] + $tmpStrat[1], $TmpNow[0] + $TmpNow[1], (($TmpNow[0] + $TmpNow[1]) - ($tmpStrat[0] + $tmpStrat[1])));
        echo "<BR>";
    }
