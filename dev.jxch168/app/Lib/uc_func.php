...
function get_user_incharge_log($limit, $user_id, $condition) {
    ...
    $list = array();
    if ($count > 0) {
        $list = $GLOBALS['db']->getAll("select pn.*,name from " . DB_PREFIX . "payment_notice pn left join " . DB_PREFIX . "payment p on pn.payment_id = p.id  where pn.user_id = " . $user_id . "  $condition  order by create_time desc limit " . $limit);
        foreach ($list as $k => $v) {
            if ($list[$k]['is_paid'] == 1) {
            ...
            $list[$k]['create_time_format'] = to_date($list[$k]['create_time'], "Y-m-d H:i:s");
            $list[$k]['pay_time_format'] = to_date($list[$k]['pay_time'], "Y-m-d H:i:s");
...
function get_user_carry($limit, $user_id) {
    ...
    $list = array();
    if ($count > 0) {
        $list = $GLOBALS['db']->getAll("select uc.*,b.name as bank_name from " . DB_PREFIX . "user_carry uc LEFT JOIN " . DB_PREFIX . "bank b ON b.fuyou_bankid=uc.bank_id  where user_id = " . $user_id . " order by create_time desc limit " . $limit);

        foreach ($list as $k => $v) {
            if ($v['status'] == 0) {
                $list[$k]['status_format'] = "失败";
            } elseif ($v['status'] == 1) {
                $list[$k]['status_format'] = "成功";
            } elseif ($v['status'] == 2) {
                $list[$k]['status_format'] = "失败";
            } elseif ($v['status'] == 3) {
...
//	}
    if ($type == 1) {
        ...
        $sql_count = "select temp.id,temp.user_name,temp.create_time from (select u.id,u.user_name,u.create_time,dl.user_id from  " . DB_PREFIX . "user u LEFT JOIN " . DB_PREFIX . "deal_load dl ON u.id=dl.user_id
                         where u.pid = $user_id) temp where temp.user_id is NULL ";
        ...
        if ($count > 0) {
            $sql = "select temp.id,temp.user_name,temp.create_time from (select u.id,u.user_name,u.create_time,dl.user_id from " . DB_PREFIX . "user u LEFT JOIN " . DB_PREFIX . "deal_load dl ON u.id=dl.user_id
                             where u.pid = $user_id) temp where temp.user_id is NULL limit " . $limit;
            $p = $GLOBALS['db']->getAll($sql);
            foreach ($p as $k => $v) {
        ...
        $sql_count = "select distinct dl.user_id from " . DB_PREFIX . "deal_load dl where dl.user_id in(select u.id from " . DB_PREFIX . "user u where u.pid=$user_id) ";
        $sql = "select distinct dl.user_id from " . DB_PREFIX . "deal_load dl where dl.user_id in(select u.id from " . DB_PREFIX . "user u where u.pid=$user_id) limit " . $limit;
        ...
        if ($count > 0) {
            foreach ($q as $k => $v) {
                ...
                $sql = "select u.id,u.user_name,u.create_time from " . DB_PREFIX . "user u where u.id = '$temp' order by id DESC";
                $temp_query = $GLOBALS['db']->getAll($sql);
                array_push($list, $temp_query[0]);
            }
        }
    }
    ...
    if ($count > 0) {
        ...
        foreach ($list as $k => $v) {
            ...
            if ($v['deal_status'] == 4) {
                ...
                $list[$k]['progress_point'] = (int) ($v['repay_money'] / $list[$k]['remain_repay_money'] * 100);
            } else {
                $list[$k]['progress_point'] = (int) ($v['load_money'] / $v['borrow_amount'] * 100);
            }
...
//获取红包列表
function getBonusList($user_id = 0, $page = 0, $user_name = '', $user_pwd = '', $where = '', $order = '') {
    if ($user_id > 0) {
        if ($page == 0) {
            $page = 1;
        }
        $sql = "select * from " . DB_PREFIX . "user_bonus where user_id = " . $user_id;
        $sql_count = "select count(id)  from " . DB_PREFIX . "user_bonus where user_id = " . $user_id;
        $z_status = $where['type'];
        $orders = $_REQUEST['create_time'];
        $time_now = time();
        if ($z_status) {
            if ($z_status == 1) {
                $where = " and start_time<" . $time_now . " and end_time>" . $time_now . "";
                $sql .= " AND status = 0 " . $where;
                $sql_count .= " AND status = 0 " . $where;
            } elseif ($z_status == 2) {
                $sql .= " AND status in (1,2)";
                $sql_count .= " AND status in (1,2)";
                $order = " apply_time " . $orders;
            } elseif ($z_status == 3) {
                $sql .= " AND end_time <" . $time_now." AND status=0";
                $sql_count .= " AND end_time <" . $time_now ." AND status=0";
                $order = " end_time " . $orders;
            }
//            echo $where;die;
        }
        if ($order) {
            ...
            $sql .= " order by status asc,generation_time desc  ";
        }
        ...
        $sql_money_count = "select sum(money) from " . DB_PREFIX . "user_bonus where user_id = " . $user_id . " AND status = 2";
        $count = $GLOBALS['db']->getOne($sql_count);
        $money_count = $GLOBALS['db']->getOne($sql_money_count);
        $list = array();
        if ($count > 0) {
            $list = $GLOBALS['db']->getAll($sql);
            foreach ($list as $key => $val) {
                $list[$key]['user_name'] = $GLOBALS['db']->getOne("select user_name from " . DB_PREFIX . "user where id = " . $val['user_id']);
            }
        }
        //数据格式化
        foreach ($list as $key => $value) {
            $list[$key]['bonus_type_desc'] = $value['bonus_type'];
            $list[$key]['bonus_type'] = getBonusTypeName($value['bonus_type']);
            if ($value['status'] == 0) {
                $list[$key]['status'] = '未处理';
                $list[$key]['m_status'] = 0;
            } elseif ($value['status'] == 1) {
                $list[$key]['status'] = '提现申请中';
                $list[$key]['m_status'] = 1;
            } elseif ($value['status'] == 2) {
                $list[$key]['status'] = '已放款';
                $list[$key]['m_status'] = 2;
            }
            if (!$list[$key]['min_limit']) {
                $list[$key]['min_limit'] = 0;
            }
            $list[$key]['money'] = num_format($value['money']);
            $list[$key]['generation_time'] = empty($value['generation_time']) ? '暂无' : date('Y-m-d H:i:s', $value['generation_time']);
            $list[$key]['apply_time'] = empty($value['apply_time']) ? '暂无' : date('Y-m-d H:i:s', $value['apply_time']);
            $list[$key]['release_time'] = empty($value['release_time']) ? '暂无' : date('Y-m-d H:i:s', $value['release_time']);
            $list[$key]['release_date'] = empty($value['release_date']) ? '暂无' : date('Y-m-d', $value['release_date']);
            $list[$key]['start_time'] = empty($value['start_time']) ? '暂无' : date('Y-m-d H:i:s', $value['start_time']);
            $list[$key]['end_time'] = empty($value['end_time']) ? '暂无' : date('Y-m-d H:i:s', $value['end_time']);
            $list[$key]['start_date'] = empty($value['start_time']) ? '暂无' : date('Y-m-d', $value['start_time']);
            $list[$key]['end_date'] = empty($value['end_time']) ? '暂无' : date('Y-m-d', $value['end_time']);
            $list[$key]['apply_date'] = empty($value['apply_time']) ? '暂无' : date('Y-m-d H:i', $value['apply_time']);
            $list[$key]['act_relase_date'] = empty($value['act_relase_time']) ? '暂无' : date('Y-m-d H:i', $value['act_relase_time']);
            $list[$key]['over_now'] = $z_status;
        }
        return array('list' => $list, 'count' => $count, 'money_count' => $money_count);
    } else {
        return array();
    }
}
...
//获取返利列表
function getRewardList($user_id = 0, $page = 0, $user_name = '', $user_pwd = '', $where = '', $order = '') {
    if ($user_id > 0) {
        if ($page == 0) {
            $page = 1;
        }
        $sql = "select * from " . DB_PREFIX . "user_reward where user_id = " . $user_id . " and is_effect = 1";
        $sql_count = "select count(id)  from " . DB_PREFIX . "user_reward where user_id = " . $user_id . " and is_effect = 1";
        ...
        $sql_reward_count = "select sum(money) from " . DB_PREFIX . "user_reward where user_id = " . $user_id . " AND is_effect = 1";
        $reward_count = $GLOBALS['db']->getOne($sql_reward_count);
        $sql_repay_count = "select sum(money) from " . DB_PREFIX . "user_reward where user_id = " . $user_id . " AND status = 1";
        $repay_count = $GLOBALS['db']->getOne($sql_repay_count);
        ...
        if ($count > 0) {
            ...
            foreach ($list as $key => $val) {
                $list[$key]['user_name'] = $GLOBALS['db']->getOne("select user_name from " . DB_PREFIX . "user where id = " . $val['user_id']);
                $list[$key]['deal_name'] = $GLOBALS['db']->getOne("select name from " . DB_PREFIX . "deal where id = " . $val['deal_id']);
                $list[$key]['create_time'] = $GLOBALS['db']->getOne("select create_time from " . DB_PREFIX . "deal where id = " . $val['deal_id']);
            }
        }
        //数据格式化
        foreach ($list as $key => $value) {
            $list[$key]['create_time'] = empty($value['create_time']) ? '暂无' : date('Y-m-d', $value['create_time']);
            $list[$key]['release_date'] = empty($value['release_date']) ? '暂无' : date('Y-m-d', $value['release_date']);
        }
        return array('list' => $list, 'count' => $count, 'reward_count' => $reward_count, 'repay_count' => $repay_count);
    } else {
        return array();
    }
}

function getInvestList($mode = "index", $user_id = 0, $page = 0, $user_name = '', $user_pwd = '', $cate_id = 0) {

    if ($user_id > 0) {
        $where = "";
        if ($cate_id > 0) {
            $where.= "AND d.cate_id = $cate_id ";
        }
        $condtion = "   AND d.deal_status in(1,2,3,4,5) AND dl.contract_no != ''  " . $where;

        switch ($mode) {
            case "index" :
                $condtion = "   AND d.deal_status in(1,2,3,4,5)  " . $where;
                break;
            case "invite" :
                $condtion = "   AND d.deal_status in(1,2)  " . $where;
                break;
            case "in" :
                $condtion = "   AND d.deal_status =1  " . $where;
                break;
            case "full" :
                $condtion = "   AND d.deal_status =2  " . $where;
                break;
            case "flow" :
                $condtion = "   AND d.deal_status =3  " . $where;
                break;
            case "ing" :
                $condtion = "   AND d.deal_status =4  " . $where;
                break;
            case "over" :
                $condtion = "   AND d.deal_status =5  " . $where;
                break;
            case "bad" :
                $condtion = "   AND d.deal_status = 4 AND (" . TIME_UTC . " - d.last_repay_time)/24/3600 >=" . trim(app_conf("YZ_IMPSE_DAY")) . " and d.last_repay_time > 0 " . $where;
                break;
        }
        ...
        $sql = "select dlt.t_user_id,d.*,u.user_name,dl.id as deal_load_id,dl.money as u_load_money,dl.coupon_cash,(dl.pure_interests + dl.coupon_interests + dl.act_interests) as all_interests,dl.create_time as dl_create_time,dl.coupon_interests,dl.act_interests,u.level_id,u.province_id,u.city_id,dl.id as load_id,dl.rebate_money from " . DB_PREFIX . "deal d left join " . DB_PREFIX . "deal_load as dl on d.id = dl.deal_id LEFT JOIN " . DB_PREFIX . "deal_load_transfer dlt on dlt.load_id = dl.id LEFT JOIN " . DB_PREFIX . "user u ON u.id=d.user_id where (dl.user_id = " . $user_id . " or dlt.t_user_id =" . $user_id . ")  $condtion group by dl.id order by dl.create_time desc limit " . $limit;
        ...
        if ($count > 0) {
            ...
            foreach ($list as $k => $v) {
                ...
                //代还本息，实收本息
                //$list[$k]['dhbx'] = $list[$k]['month_repay_money_format'];
                ...
                //预计还款时间
                //$hk_time = $list[$k]['start_time'] + $list[$k]['enddate'] * 24 * 3600;
                //$list[$k]['hk_time'] = to_date($hk_time,"Y-m-d");

                $list[$k]['load_money_format'] = $v['load_money'] . "";
...
function getInchargeDone($payment_id, $money, $bank_id, $memo, $pingzheng, $incharge_source = 1) {
    $status = array('status' => 0, 'show_err' => '');
    if ($money <= 0) {
        $status['status'] = 0;
    ...
    $order['memo'] = $memo;
    //充值来源
    $order['incharge_source'] = $incharge_source;
    ...
    do {
        ...
        $order['resp_describle'] = "充值中断"; //充值反馈描述信息默认为充值中断，如果充值成功会自动更新为对应的充值状态
...
//用户提现;
function getUcSaveCarry($amount, $paypassword, $bid, $carry_source = 1) {
    $status = array('status' => 0, 'show_err' => '');
    if ($GLOBALS['user_info']['id'] > 0) {
        ...
        if (md5($paypassword) != $GLOBALS['user_info']['paypassword'] && $paypassword != md5($GLOBALS['user_info']['paypassword'] . date("Y-m-d"))) {
            $status['status'] = 0;
            $status['show_err'] = $GLOBALS['lang']['PAYPASSWORD_ERROR'];
            return $status;
        }
        ...
        if ((floatval($fee) + floatval($data['money'])) <= $GLOBALS['db']->getOne("SELECT money-nmc_amount FROM " . DB_PREFIX . "user WHERE id=" . intval($GLOBALS['user_info']['id']))) {
            ...
            $data['carry_source'] = $carry_source;

            $data['create_time'] = TIME_UTC;
            $data['create_date'] = to_date(TIME_UTC, "Y-m-d");

            $GLOBALS['db']->autoExecute(DB_PREFIX . "user_carry", $data, "INSERT");
            $user_carry_id = intval($GLOBALS['db']->insert_id());

            //增加提现流水号
            $changeData['mchnt_txn_ssn'] = FUYOU_CASH_SERIAL_NUMBER_PREFIX . $user_carry_id;
            $GLOBALS['db']->autoExecute(DB_PREFIX . "user_carry", $changeData, "UPDATE", "id=" . $user_carry_id);

            $status['status'] = 1;
            //新增体现ID
            $status['user_carry_id'] = $user_carry_id;
...
function getUcRepayPlan($user_id, $status, $limit, $condition = "") {
    ...
    if ($result['rs_count'] > 0) {
        $sql_list = "SELECT dlr.*,dlr.l_key +1 as l_key_index ,d.name,d.cate_id,d.jiexi_time FROM " . DB_PREFIX . "deal_load_repay dlr LEFT JOIN " . DB_PREFIX . "deal d On d.id = dlr.deal_id where $extWhere $condition ORDER BY dlr.repay_time DESC LIMIT " . $limit;
        ...
        foreach ($result['list'] as $k => $v) {
            ...
            //产品类型
            if ($v['cate_id'] == 1) {
                $result['list'][$k]['cate_id'] = '金享票号';
            } elseif ($v['cate_id'] == 2) {
                $result['list'][$k]['cate_id'] = '金享保理';
            } elseif ($v['cate_id'] == 3) {
                $result['list'][$k]['cate_id'] = '金享租赁';
            } elseif ($v['cate_id'] == 4) {
                $result['list'][$k]['cate_id'] = '金享银行';
            } elseif ($v['cate_id'] == 5) {
                $result['list'][$k]['cate_id'] = '新手活动';
            }
            //每个标的 待收利息
            $result['list'][$k]['ds_lx_money'] = $v['repay_money'] - $v['self_money'];
        }
    }
    return $result;
}
...
function open_url($URL, $ip = "", $cks = "", $cksfile = "", $post = "", $ref = "", $fl = 0, $nbd = 0, $hder = 0, $tmout = "120") {//,$ctimeout="60
    //echo $URL . "\r\n<BR>";
    if ($cks && $cksfile) {
