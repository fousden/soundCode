...
class paymentModule extends SiteBaseModule{
    ...
    public function pay(){
        ...
        if ($payment_notice) {
            if ($payment_notice['is_paid'] == 0) {
                ...
                //$incharge_url = FUYOU_URL . '500002.action';
                //incharge_skip_url($incharge_url,$parameter);
                //die();
	...
	public function carry_tip()
    {
        $user_carry = $GLOBALS['db']->getRow("select * from " . DB_PREFIX . "user_carry where id = " . intval($_REQUEST['id']));
        $GLOBALS['tmpl']->assign("user_carry", $user_carry);
        $GLOBALS['tmpl']->display("page/user_carry_tip.html");
    }
