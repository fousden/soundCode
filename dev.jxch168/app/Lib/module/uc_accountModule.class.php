...
class uc_accountModule extends SiteBaseModule {
	...
	$form = strim($_REQUEST['from']);
        //判断是否绑卡开户过
        $user_bank_num = $GLOBALS["db"]->getOne("SELECT count(id) FROM ".DB_PREFIX."user_bank WHERE user_id = '".$GLOBALS["user_info"]["id"]."'");
	$GLOBALS['tmpl']->assign("user_bank_num", $user_bank_num);
    ...
    public function changeMobile(){
        ...
        $fuyou = new fuyou();
        $change_mobile_code = $fuyou->getChangeMobileCode($GLOBALS['user_info']);
        die($change_mobile_code);
    }
    
    //更换手机号回调
    function changeMobileNotice(){
        require_once APP_ROOT_PATH . "system/payment/fuyou.php";
        $fuyou = new fuyou();
        $request =$_REQUEST;
        //更换手机号回调
        ...
        if($return["status"] == 1){
            showSuccess("<div style='font-size:16px;color:#C40000'>更换手机号成功！</div>", 0,CLI_DOMAIN."member.php?ctl=uc_account&act=security");die();
        }else{
            showErr("<div style='font-size:16px;color:#C40000'>"."更换手机号失败，失败原因：".$return["resp_desc"]."！</div>", 0,CLI_DOMAIN."member.php?ctl=uc_account&act=security");die();
        }
    }
