...
class ajaxModule extends SiteBaseModule {
    ...
    function check_user() {
        ...
        if (preg_match("/^\d{7}$/", $_REQUEST['val'])) {
            $ad_id = $_REQUEST['val'];
            if ($GLOBALS['db']->getOne("SELECT id FROM " . DB_PREFIX . "admin WHERE work_id ={$ad_id}") > 0) {
                $result['status'] = 1;
                ajax_return($result);
            }
        }
    ...
    function checkBidMoney() {
        ...
        $coupon_info = $GLOBALS['db']->getRow("SELECT * FROM " . DB_PREFIX . "user_coupon WHERE status = 0 AND id ='" . $coupon_id . "'");
        if ($coupon_info) {
            if ($coupon_info['coupon_type'] == 1) {
                //收益券
                $result['status'] = 1;
                $result['face_value'] = $coupon_info['face_value'];
                $result['min_limit'] = $coupon_info['min_limit'];
    ...
    //ajax检测用户信息真实性
    public function check_user_real_info() {
        $verify_status = $GLOBALS['db']->getRow("select idcardpassed,mobilepassed,paypassword,real_name from " . DB_PREFIX . "user where id = " . $GLOBALS['user_info']['id']);
        //实名认证未通过，请完成实名认证再充值
        if ($verify_status['idcardpassed'] != 1 || !$verify_status['real_name']) {
            $result['status'] = 0;
            $result['skip_url'] = '/index.php?ctl=uc_account&act=security';
            $result['info'] = "<div style='font-size:18px;color:#C40000'>您尚未实名认证！</div>请先完成实名认证再进行交易。";
            ajax_return($result);
        }
        ...
        if ($verify_status['mobilepassed'] != 1) {
            $result['status'] = 0;
            $result['skip_url'] = '/index.php?ctl=uc_account&act=security';
            $result['info'] = "<div style='font-size:18px;color:#C40000'>您尚未手机认证！</div>请先完成手机认证再进行交易。";
            ajax_return($result);
        }
        //银行卡必须绑定，而且只能绑定一张，否则无法充值，需重新绑定！
        $bank_num = $GLOBALS['db']->getOne("select count(id) from " . DB_PREFIX . "user_bank where user_id = " . $GLOBALS['user_info']['id']);
        if ($bank_num['num'] != 1) {
            $result['status'] = 0;
            $result['skip_url'] = '/index.php?ctl=uc_money&act=bank';
            $result['info'] = "<div style='font-size:16px;color:#C40000'>您尚未绑定银行卡！</div>请先绑定银行卡再进行交易。";
            ajax_return($result);
        }
        //支付密码是否设置
        if (!$verify_status['paypassword']) {
            $result['status'] = 0;
            $result['skip_url'] = '/index.php?ctl=uc_account&act=security';
            $result['info'] = "<div style='font-size:16px;color:#C40000'>您尚未设置支付密码！</div>请先设置支付密码再进行交易。";
            ajax_return($result);
        }
        //首先判断金享财行个人账户可用余额
        $personal_money = $GLOBALS['db']->getOne("select money from " . DB_PREFIX . "user where id = " . $GLOBALS['user_info']['id'] . " and is_delete = 0 and is_effect = 1");
        $bid_money = $_REQUEST['check_money'];
        if ($bid_money <= 0) {
            $result['status'] = 0;
            $result['skip_url'] = '';
            $result['info'] = "<div style='font-size:16px;color:#C40000'>请输入正确的投资金额！</div>";
        }
        if ($_REQUEST['coupon_id']) {
            $coupon_money = $GLOBALS['db']->getOne("select face_value from " . DB_PREFIX . "user_coupon where id ={$_REQUEST['coupon_id']} and coupon_type=2");
            $bid_money = $bid_money - $coupon_money;
        }
        if ($personal_money <= 0 || $bid_money > $personal_money) {
            $result['status'] = 0;
            $result['skip_url'] = '/index.php?ctl=uc_money&act=incharge';
            $result['info'] = "<div style='font-size:16px;color:#C40000'>账户余额不足,请及时充值</div>";
            ajax_return($result);
        }
        $result['status'] = 1;
        ajax_return($result);
    ...
    public function get_register_verify_code() {
        str_replace('index.php?ctl=user&act=register', '', basename($_SERVER['HTTP_REFERER']), $cnt);
        if (empty($cnt)) {
            MO("MobileBlacklist")->addMobile($_POST['user_mobile']);
            $data['status'] = 0;
            $data['info'] = '手机异常，请安正常途径操作';
            ajax_return($data);
        }
        ...
        $result = send_verify_sms_new($user_mobile, 'TPL_SMS_VERIFY_CODE', $verify_data['verify_code'], null, true, true);
    ...
    public function get_re_pwd_verify_code() {
        ...
        $result = send_verify_sms($user_mobile, $verify_data['verify_code'], $users, false, true);
        $data['status'] = $result['status'];
        if ($data['status'] == 0) {
            $data['info'] = $result['msg'];
        } else {
    ...
    public function get_verify_code() {
        ...
        send_verify_sms_new($user_mobile, 'TPL_SMS_BIND_MOBILE_VERIFY_CODE', $verify_data['verify_code'], $GLOBALS['user_info']);
    ...
    public function check_verify_code() {
        ...
        if ($GLOBALS['user_info']['mobile']) {
            ...
            $resp_desc = $GLOBALS['lang']['MOBILE_BIND_SUCCESS'];
        }
        if ($GLOBALS['db']->getOne("SELECT count(*) FROM " . DB_PREFIX . "mobile_verify_code WHERE mobile='" . $user_mobile . "' AND verify_code='" . $verify . "' AND create_time + " . SMS_EXPIRESPAN . " > " . TIME_UTC . " ") == 0) {
            ...
            $GLOBALS['db']->query("update " . DB_PREFIX . "user set mobile='" . $user_mobile . "',fuiou_account='" . $user_mobile . "',mobilepassed=1, bind_verify = '', verify_create_time = 0 where id = " . intval($GLOBALS['user_info']['id']));
            if ($GLOBALS['db']->affected_rows() > 0) {
                showSuccess($resp_desc, $ajax);
            } else {
                showErr("绑定失败", $ajax);
            }
        }
    ...
    public function get_paypwd_verify_code() {
        ...
        $result = send_verify_sms_new($user_mobile, 'TPL_SMS_PAYMENT_PWD_VERIFY_CODE', $verify_data['verify_code'], $GLOBALS['user_info'], false, true);
        $data['status'] = $result['status'];
        if ($data['status'] == 0) {
            $data['info'] = $result['msg'];
        } else {
    ...
    public function cancel() {
        ...
        if (!$GLOBALS['user_info']) {
            ...
            $sql = "delete from " . DB_PREFIX . "deal_collect where user_id=" . intval($GLOBALS['user_info']['id']);
//                      file_put_contents('1.txt', json_encode($GLOBALS['user_info']));
//                     / file_put_contents('1.txt', $sql);
    ...
    public function send_mobile_verify_code() {
        ...
        //使用立即发送方式
        $result = send_verify_sms($mobile, $verify_code, null, true, true); //
        $data['status'] = $result['status'];
