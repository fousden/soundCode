...
class creditModule extends SiteBaseModule {
    ...
    function credit_save(){
    	...
    	if($type == "credit_identificationscanning"){
    		...
    		$u_c_data['idno'] = strtoupper(strim($_REQUEST['idno']));//如果身份证号码中有字母则转换成大写
    		...
    		$u_c_data['sex'] = intval($_REQUEST['sex']);
                if($u_c_data['sex'] == -1){
    			showErr("请选择您的性别！",$this->is_ajax);
    		}
    		$u_c_data['byear'] = $_REQUEST['byear'];
    		$u_c_data['bmonth'] = $_REQUEST['bmonth'];
    		$u_c_data['bday'] = $_REQUEST['bday'];
    		$u_c_data['idcardpassed'] = 1;
                $u_c_data['idcardpassed_time'] = time();
		MO("ActivityConf")->ActivityConfByType(1,intval($GLOBALS['user_info']['id']));
                //实名认证状态默认为已通过
    	...
    	if($this->is_ajax==1)
    		showSuccess("恭喜您，您的实名认证已成功!",$this->is_ajax);
    	else
