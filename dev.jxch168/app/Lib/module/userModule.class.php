...
class userModule extends SiteBaseModule {
    ...
    public function register() {
        ...
        $service_channel = isset($_GET['s']) ? $_GET['s'] : ' ';
        ...
        parse_str($_SERVER['QUERY_STRING'], $urlPara);

        if (isset($urlPara['r'])) {
            //  $is_sale_invite = mb_substr($urlPara['r'], 0, 4);
            //  var_dump($is_sale_invite,$tmp);
            if (preg_match("/^\d{7}$/", $urlPara['r'])) { //
                $referer = $urlPara['r'];
            } else {
                $referer = strim(base64_decode($urlPara['r']));
            }
        }
        $GLOBALS['tmpl']->assign("referer", $referer);
        //如果有推广链接有r且前缀是‘jxch’开头的传值就是代理商销售的推广链接，把此注册用户归到他名下
        if (!$admin_id) {
            $admin_id = 0;
        }

        $GLOBALS['tmpl']->assign("admin_id", $admin_id);
        $GLOBALS['tmpl']->assign("mobile", $mobile);
        $GLOBALS['tmpl']->assign("service_channel", $service_channel);
    ...
    public function doregister() {
        ...
        $is_sale_invite = $_REQUEST['referer'];

        if (preg_match("/^\d{7}$/", $is_sale_invite)) {
            $res = $GLOBALS['db']->getOne("SELECT id FROM " . DB_PREFIX . "admin WHERE work_id={$is_sale_invite}");
            $user_data['admin_id'] = $res;
        }
        ...
        //富友开户时用到的账户名 默认为用户手机号码
        $user_data["fuiou_account"] = $user_data["mobile"];
        ...
        if ($res['status'] == 1) {
            ...
            MO('BonusMiddel')->middel2formal(array('id' => $user_id, 'user_name' => $user_data['user_name'], 'mobile' => $user_data['mobile']));
            MO('CouponMiddle')->middel2formal(array('id' => $user_id, 'user_name' => $user_data['user_name'], 'mobile' => $user_data['mobile']));
            //注册成功后短信通知
            MO('User')->send_msg_register($user_data['mobile'], 1);

            //更新来路
            $search_channel= es_cookie::get("search_channel")? es_cookie::get("search_channel"):'public';
                $GLOBALS['db']->query("update " . DB_PREFIX . "user set referer = '" . $GLOBALS['referer'] . "', `search_channel` = '" . $search_channel. "' where id = " . $user_id);
            MO("Channel")->updateChannel($user_data['mobile'], $user_id);
            $user_info = $GLOBALS['db']->getRow("select * from " . DB_PREFIX . "user where id = " . $user_id);
            if ($_REQUEST['opt'] == 'jump') {
                $result = do_login_user($user_data['user_name'], $user_data['user_pwd']);
                $GLOBALS['tmpl']->assign('$user_info', $result);
            ...
            if ($user_info['is_effect'] == 1) {
                ...
                app_redirect(url("index", "user#steptwo") . '&s=' . es_cookie::get("search_channel"));
            } else {
    ...
    public function login() {
        ...
        $auto_name = es_cookie::get('auto_name');
        //$auto_pwd = es_cookie::get('auto_pwd');
        $GLOBALS['tmpl']->assign("login_type", true);
        $GLOBALS['tmpl']->assign("user_name", $auto_name);
        //$GLOBALS['tmpl']->assign("user_pwd", $auto_pwd);
    ...
    public function dologin() {
        ...
        if ($result['status']) {
            ...
            if (intval($_POST['auto_login']) == 1) {
                ...
                es_cookie::set("user_pwd", $user_data['user_pwd'], 3600 * 24 * 30);
                es_cookie::set("auto_name", $_POST['email'], 3600 * 24 * 30);
                //es_cookie::set("auto_pwd", $_POST['user_pwd'], 3600 * 24 * 30);
            } else {
                es_cookie::delete("auto_name");
                //es_cookie::delete("auto_pwd");
            }
            if ($ajax == 0 && trim(app_conf("INTEGRATE_CODE")) == '') {
                ...
                if ($s_user_info['ips_acct_no'] == "" && app_conf("OPEN_IPS")) {
                    ...
                    if ($ajax == 1) {
                        ...
                        $return['data'] = $result['msg'];
                        $return['jump'] = $jump_url;
                        $login_data['user_name'] = $s_user_info['user_name'];
                        $login_data['login_time'] = time();
                        $login_data['terminal'] = 1;
                        $login_data['ip_address'] = get_client_ip();
                        $GLOBALS['db']->autoExecute(DB_PREFIX . "user_login_log", $login_data, "INSERT");
                        ajax_return($return);
                    } else {
                        $GLOBALS['tmpl']->assign('integrate_result', $result['msg']);
                        showSuccess($GLOBALS['lang']['LOGIN_SUCCESS'], $ajax, $jump_url);
    ...
    public function steptwo() {
        ...
        $PINGANPROVISIONVALUE = $GLOBALS['db']->getOne("select value from " . DB_PREFIX . "conf where name = 'PINGANPROVISION'");
        $GLOBALS['tmpl']->assign("PINGANPROVISIONVALUE", $PINGANPROVISIONVALUE);
    ...
    public function do_re_name_id() {
        $GLOBALS['tmpl']->assign("do_real_name", true);
        $id = $GLOBALS['user_info']['id'];
        $real_name = strim($_REQUEST['real_name']);
        $idno = strim($_REQUEST['idno']); //如果身份证号码中有字母则转换成大写
        $idno = strtoupper($idno);
        $sex = strim($_REQUEST['sex']);
        //判断是否勾选投保协议
        $checked = intval($_REQUEST['agreement']);
        $GLOBALS['tmpl']->assign("checked", $checked);
        ...
        $id_num = $GLOBALS['db']->getOne("SELECT count(*) FROM " . DB_PREFIX . "user where idno = '$idno' and id<> $id ");
        //判断该实名是否存在
        if ($id_num > 0) {
        ...
        if ($user = $GLOBALS['db']->getRow("select * from " . DB_PREFIX . "user where id =" . $id)) {
            ...
            // 如果选择了投保在提交的时候需要弹出一个平安LOGO和介绍界面
            if ($checked == 1) {
                $data['status'] = 1;
                $GLOBALS['db']->autoExecute(DB_PREFIX . "user", $data, "UPDATE", "id=" . $id);
            }
            MO("ActivityConf")->ActivityConfByType(1, $user['id']);
            //实名认证状态默认为已通过
            $user_info_re['idcardpassed'] = 1;
            $user_info_re['idcardpassed_time'] = time();
            if ($user_type == 1) {
            ...
            if (app_conf("OPEN_IPS") == 1) {
                showSuccess("验证成功", 0, APP_ROOT . "/index.php?ctl=collocation&act=CreateNewAcct&user_type=0&user_id=" . $id, 0, $checked);
            } else {
                //实名送积分
                MO('UserScore')->add_score($id,5);
                
                showSuccess("注册成功", 0, APP_ROOT . "/", 0, $checked);
            }
        } else {
            showErr("该用户尚未注册");  //尚未注册
        }
//        if($_GET['act']==do_re_name_id){
//        }
//        else{
//            $do_real_name = false;
//        }
