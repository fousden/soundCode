...
function get_deal($id = 0, $is_effect = 1)
{
    ...
    if ($id == 0) {  //有ID时不自动获取
        ...
        $deal = $GLOBALS['db']->getRow("select *,(load_money/borrow_amount*100) as progress_point from " . DB_PREFIX . "deal where id = " . intval($id) . "  and is_delete <> 1  $ext");
    }
...
function get_deal_list_mobile($limit = "", $cate_id = 0, $where = '', $orderby = '', $user_name = '', $user_pwd = '', $is_all = false)
{

    $time = TIME_UTC;

    $count_sql = "select count(1) from " . DB_PREFIX . "deal where 1=1 ";
    if ($is_all == false)
    ...
    $sql = "select id,name,sub_name,rate,repay_time,repay_time_type,risk_rank,min_loan_money,
			loantype,jiexi_time,qixi_time,last_mback_time,benxibaozhang,agency_id,
			start_time,enddate,titlecolor,apart_borrow_amount,borrow_amount,load_money,create_time,
			cate_id,type_id,deal_status,next_repay_time,impose_fee_day1,impose_fee_day2,manage_impose_fee_day1,
			manage_impose_fee_day2,publish_wait,
			start_time as last_time,floor((load_money/borrow_amount*100)) as progress_point,yield_ratio,
			(start_time + enddate*24*3600 - " . $time . ") as remain_time $extfield from " . DB_PREFIX . "deal where 1 = 1 ";
    ...
    if ($deals_count > 0) {
        ...
        if ($deals) {
            foreach ($deals as $k => $deal) {
                ...
                if ($deal["agency_info"]) {
                    $deal["guarantee"] = $deal["agency_info"]["user_name"];
                } else {
                    $deal["guarantee"] = "";
                }

                unset($deal["agency_info"]);
                unset($deal["cate_info"]);
                unset($deal["type_info"]);
                $deals[$k] = $deal;
            }
        }
    } else {
...
function get_deal_list($limit = "", $cate_id = 0, $where = '', $orderby = '', $user_name = '', $user_pwd = '', $is_all = false)
{
    ...
    $count_sql = "select count(1) from " . DB_PREFIX . "deal where is_effect=1 AND  is_delete=0  AND deal_status in (1,2,4,5)";
    if ($is_all == false)
        $count_sql.=" and is_effect = 1 and is_delete = 0 ";
    ...
    $sql = "select *,start_time as last_time,(load_money/borrow_amount*100) as progress_point,(start_time + enddate*24*3600 - " . $time . ") as remain_time $extfield from " . DB_PREFIX . "deal where deal_status in (1,2,4,5)";
    if ($is_all == false)
        $sql.=" and is_effect = 1 and is_delete = 0 ";
...
function format_deal_item(&$deal, $user_name = "", $user_pwd = "")
{
    ...
    $deal['rate_foramt'] = num_format($deal['rate']);
    ...
    $deal['rate_foramt_w'] = num_format($deal['rate']) . "%";
    ...
    if ($deal['deal_status'] >= 4) {
        ...
        if ($deal['deal_status'] == 4) {
            ...
            $has_repay_money = $GLOBALS['db']->getOne("SELECT sum(true_repay_money) FROM " . DB_PREFIX . "deal_load_repay WHERE deal_id=" . $deal['id'] . " AND repay_time=" . $deal['next_repay_time']);

            $deal['true_month_repay_money']      = $deal['true_month_repay_money'] - num_format($has_repay_money);
            $deal['true_last_month_repay_money'] = $deal['true_last_month_repay_money'] - num_format($has_repay_money);
...
function get_deal_load_list($deal)
{
    ...
    $tmp_has_repay_money = $GLOBALS['db']->getAll("SELECT repay_id,deal_id,user_id,user_name,real_name,mobile,sum(true_repay_money) as has_repay_money FROM " . DB_PREFIX . "deal_load_repay d left join " . DB_PREFIX . "user u on d.user_id = u.id WHERE deal_id=" . $deal['id'] . " AND has_repay=1 GROUP BY repay_id");
    $tmp_repay_money     = $tmp_has_repay_money;
    ...
    foreach ($deal_repay_list as $k => $v) {
        ...
        //查询用户信息
        foreach ($tmp_repay_money as $ke => $vl) {
            if ($v['id'] == $vl['repay_id']) {
                $loan_list[$i]['mobile']      = $vl['mobile'];
                $loan_list[$i]['user_name']   = $vl['user_name'];
                $loan_list[$i]['real_name']   = $vl['real_name'];
                $deal_infos                   = $GLOBALS['db']->getRow("SELECT name,yield_ratio,deal_sn FROM " . DB_PREFIX . "deal where id=" . $vl['deal_id']);
                $loan_list[$i]['deal_name']   = $deal_infos['name'];
                $loan_list[$i]['yield_ratio'] = intval($deal_infos['yield_ratio']);
                $loan_list[$i]['deal_sn']     = intval($deal_infos['deal_sn']);
            }
        }
        ...
        $loan_list[$i]['manage_money_rebate'] = 0;
        ...
        if ($v['has_repay'] == 1) {
            ...
            $loan_list[$i]['manage_impose_money'] = 0;
            ...
            $loan_list[$i]['month_has_repay_money_all'] = $loan_list[$i]['month_has_repay_money'] ;
...
//                $manage_impose_fee = floatval($manage_impose_fee);
            ...
            $loan_list[$i]['month_need_all_repay_money'] = $loan_list[$i]['month_repay_money'];
...
function get_deal_user_load_list($deal_info, $user_id = 0, $lkey = -1, $ukey = -1, $true_time = 0, $get_type = 0, $r_type = 0, $limit = "")
{
    ...
    //必须有合同号 必须是手动投标
    $extW .= " AND dl.is_auto = 0 AND dl.contract_no != '' ";
    
    if($deal_info['deal_load_id']){
        $extW.=" AND dl.id=".$deal_info['deal_load_id']."";
    }
    if (!empty($limit)) {
        $limit = " limit " . $limit;
        $sql = "SELECT count(*) FROM " . DB_PREFIX . "deal_load_repay dlr left join ".DB_PREFIX."deal_load dl on dl.id = dlr.load_id " .
                " WHERE dlr.deal_id=" . $deal_info['id'] . " $extW";
        $count           = $GLOBALS['db']->getOne($sql);
        $result['count'] = $count;
    }

    $sql = "SELECT dlr.*,dl.pMerBillNo,dl.money,dl.is_winning,dl.income_type,income_value,u.ips_acct_no,u.mobile,u.fuiou_account,u.email,u.user_name,u.real_name,tu.ips_acct_no as t_ips_acct_no,tu.id as t_user_id,tu.user_name as t_user_name,tu.mobile as t_mobile,tu.email as t_email  FROM " . DB_PREFIX . "deal_load_repay dlr " .
    ...
    foreach ($load_users as $k => $v) {
        ...
        $item['real_name']      = $v['real_name'];
        $item['email']          = $v['email'];
        $item['mobile']         = $v['mobile'];
        $item['fuiou_account']  = $v['fuiou_account'];
        ...
        if ($v['has_repay'] == 0) {
            ...
            $item['month_has_repay_money_all'] = $item['month_repay_money'] ;
        }

        //$item['expect_earnings'] = $v['interest_money'] - $v['manage_money'] - $v['manage_interest_money'] + $v['reward_money'];
        $item['expect_earnings'] = $v['interest_money'] + $v['reward_money'];
        if ($item['has_repay'] == 1) {
            ...
            $item['true_earnings'] = $v['true_interest_money'] + $v['true_reward_money'];
        } else {
           // $item['expect_earnings'] += $item['impose_money'];
            $item['expect_earnings'] += 0;
...
//自动投标检测
function auto_check_dobid2($deal_id, $bid_money, $user_info, $is_pc = 0)
{
    $root           = array();
...
function check_dobid2($deal_id, $bid_money, $bid_paypassword, $is_pc = 0)
{
    ...
    $user_paypassword=$GLOBALS['user_info']['paypassword'];

    if (md5($bid_paypassword) != $user_paypassword && $bid_paypassword != md5($user_paypassword.date("Y-m-d")) && $bid_paypassword != md5($user_paypassword.date("Y-m-d", strtotime("+1 year"))) && $bid_paypassword != md5($user_paypassword.date("Y-m-d", strtotime("-1 day"))) ) {
    ...
    $total_bid_money = $GLOBALS['db']->getOne("select sum(money) from " . DB_PREFIX . "deal_load where is_auto = 0 AND contract_no != '' AND deal_id = '" . $deal_id . "' AND user_id = '" . $GLOBALS['user_info']['id'] . "'" );
    $total_bid_money = $total_bid_money ? $total_bid_money : 0;
    //投资金额超过该标的限制金额
    if (floatval($deal['max_loan_money']) > 0) {
        if ($total_bid_money >= floatval($deal['max_loan_money'])) {
            //$root["show_err"] = $GLOBALS['lang']['BID_MONEY_THOUSAND_NOT_TRUE'];
            $root["show_err"] = "投资金额超过该标的最高限制金额" .$deal['max_loan_money'] . "元" ;
            return $root;
        }else{
            $total_moneys = $bid_money + $total_bid_money;
            if($total_moneys > floatval($deal['max_loan_money'])){
                $root["show_err"] = "投资金额超过该标的最高限制金额,剩余可投金额".($deal['max_loan_money'] - $total_bid_money ). "元";
                return $root;
            }
        }
    }

    $deal_load_list_sql = "SELECT deal_id FROM " . DB_PREFIX . "deal_load WHERE  is_auto = 0 AND contract_no != '' AND user_id=" . $GLOBALS['user_info']['id'];
    $deal_load_list    = $GLOBALS['db']->getAll($deal_load_list_sql);
    $num = 0;
    foreach ($deal_load_list as $key => $value) {
        $deal_id_sql = "SELECT cate_id FROM " . DB_PREFIX . "deal WHERE id=" . $value['deal_id'];
        $cate_id    = $GLOBALS['db']->getOne($deal_id_sql);
        $deal_cate_sql = "SELECT uname FROM " . DB_PREFIX . "deal_cate WHERE id=" . $cate_id;
        $uname    = $GLOBALS['db']->getOne($deal_cate_sql);
        if($uname == "activity"){
            $num += 1 ;
        }
    }
    if ($deal['cate_info']['uname'] == 'activity') {
        if($num >= 1){
            $root["show_err"] = "新手任务标的限投1次";
                return $root;
        }
    }
    /*if ($deal['cate_info']['uname'] == 'activity') {
        ...
        //判断 新手任务标的限投1次
        $has_bid_count_sql = "SELECT count(*) FROM " . DB_PREFIX . "deal_load WHERE  is_auto = 0 AND contract_no != '' AND deal_id=" . $deal_id . " AND user_id=" . $GLOBALS['user_info']['id'];
        $has_bid_count     = $GLOBALS['db']->getOne($has_bid_count_sql);
        if ($has_bid_count >= 1) {
            //$root["show_err"] = $GLOBALS['lang']['DEAL_BID_THOUSAND_MAX_COUNT'];
            $root["show_err"] = "新手任务标的限投1次";
            return $root;
        }
    }
...
//自动投标
function autodobid2($deal_id, $bid_money, $user_info, $is_pc = 0, $payHandle = true)
{
    $root = auto_check_dobid2($deal_id, $bid_money, $user_info, $is_pc);
    ...
    $root["status"]  = 0;
    $bid_money       = floatval($bid_money);
    $bid_paypassword = strim($user_info['paypassword']);
    ...
    //纯利息入订单表
    $insertdata['pure_interests'] = num_format(($deal['rate'] / 100 / 360) * $bid_money * $deal['repay_time'] * $deal['yield_ratio']);
    //订单来源
    $insertdata['order_source'] = 1;
    //自动投标时间处理
    //获取最后一次投标时间
    $last_time = $GLOBALS['db']->getOne("select create_time from " . DB_PREFIX . "deal_load where 1 = 1 AND deal_id = ".$deal["id"]." ORDER BY create_time DESC limit 1 ");
    if($last_time){
        $insertdata["create_time"] = mt_rand($last_time, time());
    }else{
        $insertdata["create_time"] = mt_rand($deal["start_time"], time());
    }
    $insertdata["create_date"] = date("Y-m-d",$insertdata["create_time"]);    

    $GLOBALS['db']->autoExecute(DB_PREFIX . "deal_load", $insertdata, "INSERT");
    $load_id = $GLOBALS['db']->insert_id();
    if ($load_id > 0) {
        //更改资金记录
        $msg = '[<a href="' . $root['deal']['url'] . '" target="_blank">' . $root['deal']['name'] . '</a>]的自动投标,付款单号' . $load_id;
        require_once APP_ROOT_PATH . "system/libs/user.php";
        modify_account(array('money' => -$bid_money, 'lock_money' => $bid_money), $user_info['id'], $msg, 2);
        //更新标的信息
        $deal = $GLOBALS['db']->getRow("select *,(load_money/borrow_amount*100) as progress_point from " . DB_PREFIX . "deal where id = '" . intval($deal_id) . "'  and is_delete <> 1  and is_effect = 1 ");
        if ($deal) {
            if ($deal['deal_status'] != 3 && $deal['deal_status'] != 5) {
...
function dobid2($deal_id, $bid_money, $bid_paypassword, $is_pc = 0 ,$order_source = 1,$coupon_id = 0)
{
    ...
    $insertdata['pure_interests'] = num_format(($deal['rate'] / 100 / 360) * $bid_money * $deal['repay_time'] * $deal['yield_ratio']);
    //订单来源
    $insertdata['order_source'] = $order_source;
    //是否使用优惠券
    if($coupon_id){
        $insertdata['coupon_id']     = $coupon_id;
        //优惠券收益 资金计算
        $coupon_info = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."user_coupon where id = '".$coupon_id."'");
        //判断是否了优惠券
        $used_coupon_info = get_coupon_money($deal,$coupon_info,$bid_money);
        if($used_coupon_info){
            if($used_coupon_info['coupon_type'] == 1){
                //收益券收益入库
                $insertdata['coupon_interests'] = num_format($used_coupon_info['coupon_interests']);
            }else if($used_coupon_info['coupon_type'] == 2){
                //抵现券 抵除现金
                $insertdata['coupon_cash'] = $bid_money - $used_coupon_info['act_bid_money'];
                $bid_money = $used_coupon_info['act_bid_money'];
            }
        }
    }
    ...
    if ($load_id > 0) {
        ...
        if($GLOBALS["user_info"]["user_name"] == "李明明" && $GLOBALS["user_info"]['is_auto'] == 1){
            //累加需要充值的金额
            $msgs ='自动投标充值'.$bid_money.'，标名【'.$deal['name'].'】标号【'.$deal['id'].'】';
            modify_account(array('money'=>$bid_money),$GLOBALS["user_info"]['id'],$msgs,13);
            ...
            //如果存在优惠券 则更新优惠券状态
            if($coupon_info){
              $GLOBALS['db']->autoExecute(DB_PREFIX . "user_coupon", array('status' => 1,'load_id' => $load_id), "UPDATE", ' `id` ="' . $coupon_info['id'] . '"  ');
            }
            //更新标的信息
            $deal = $GLOBALS['db']->getRow("select *,(load_money/borrow_amount*100) as progress_point from " . DB_PREFIX . "deal where id = '" . intval($deal_id) . "'  and is_delete <> 1  and is_effect = 1 ");
            if ($deal) {
            ...
            $root["status"] = 6; //0:出错;1:正确;
        }else{
            require_once APP_ROOT_PATH . "system/payment/fuyou.php";
            $fuyou = new fuyou();

            //$arr   = $fuyou->preAuthAction($GLOBALS['user_info']['mobile'], $bid_money, $load_id);
            $arr   = $fuyou->preAuthAction($GLOBALS['user_info']['fuiou_account'], $bid_money, $load_id);//使用富友账户信息
            if ('0000' == $arr->plain->resp_code) {
                //更改资金记录
                $msg              = '[<a href="' . $root['deal']['url'] . '" target="_blank">' . $root['deal']['name'] . '</a>]的投标,付款单号' . $load_id;
                modify_account(array('money' => -$bid_money, 'lock_money' => $bid_money), $GLOBALS['user_info']['id'], $msg, 2);
                file_put_contents(APP_ROOT_PATH . 'log/fuyou/' . date('Y-m-d') . '_preAuth.log', "[投标预授权成功，投标成功，成功的deal_load投标记录的ID为" . $load_id . "，投标人ID为" . $GLOBALS['user_info']['id'] . ",投标人用户名为" . $GLOBALS['user_info']['user_name'] . ",投标金额为" . $data['money'] . "];return[返回信息" . json_encode($arr) . "];[" . date('Y-m-d H:i:s') . "]\r\n", FILE_APPEND);
                //如果存在优惠券 则更新优惠券状态
                if($coupon_info){
                  $GLOBALS['db']->autoExecute(DB_PREFIX . "user_coupon", array('status' => 1,'load_id' => $load_id), "UPDATE", ' `id` ="' . $coupon_info['id'] . '"  ');
                }
                ...
                $tmp              = $GLOBALS['db']->autoExecute(DB_PREFIX . "deal_load", array('contract_no' => $arr->plain->contract_no), "UPDATE", ' `id` ="' . $load_id . '"  ');
                //投标成功
                $root["show_err"] = $GLOBALS['lang']['DEAL_BID_SUCCESS'];
                $root['load_id']=$load_id;
                ...
                //删除投资记录 投标失败
                $res = $GLOBALS['db']->autoDelete(DB_PREFIX . "deal_load", $load_id);
                if (!$res) {
                    //查询自动投标用户信息 在100个自动投标用户中随机产生一个用户进行自动投标
                    $user_counts = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."user where is_auto = 1");
                    $auto_user_info = $user_counts[mt_rand(0, count($user_counts) - 1)];
                    $GLOBALS['db']->autoExecute(DB_PREFIX . "deal_load", array('user_id'=>$auto_user_info["id"],'user_name'=>$auto_user_info["user_name"],'is_auto' => 1), "UPDATE", ' `id` ="' . $load_id . '"  ');
                }
                file_put_contents(APP_ROOT_PATH . 'log/fuyou/' . date('Y-m-d') . '_preAuth.log', "[投标预授权失败，投标失败，失败的deal_load投标记录的ID为" . $load_id . "，投标人ID为" . $GLOBALS['user_info']['id'] . ",投标人用户名为" . $GLOBALS['user_info']['user_name'] . ",投标金额为" . $data['money'] . "];return[返回信息" . json_encode($arr) . "];[" . date('Y-m-d H:i:s') . "]\r\n", FILE_APPEND);
                $root["show_err"] = $GLOBALS['lang']['DOBIE_PRE_ERROR'];
                $root["status"]   = 4; //0:出错;1:正确;
            }
        }
    } else {
        $root["show_err"] = $GLOBALS['lang']['DOBIE_SAVE_ERROR'];
        $root["status"]   = 5;
    }
    if($root['status']==6){
        //投资成功后发送微信的模板消息
//        MO('Weixin')->templateMsgData("投标成功",$GLOBALS['user_info']['id'],['load_id'=>$load_id]);
    }
    return $root;
}
...
function bid_calculate($data)
{
   ...
   // $all_manage_money = $all_manage_money * floatval($deal['yield_ratio']);
    ...
    //number_format($deal_rs['remain_repay_money'] - floatval($data['money']) - floatval($all_manage_money),2);
    //千倍收益
    //$pre_income = ($deal_rs['remain_repay_money'] - floatval($data['money']) - floatval($all_manage_money)) * intval($data['yield_ratio']);
    //$pre_income = ($deal_rs['remain_repay_money'] - $deal['borrow_amount']) * intval($data['yield_ratio']);
    $pre_income = $data['money'] * $deal['rate']/360/100 * $data['repay_time'] * intval($data['yield_ratio']);

    //是否使用优惠券
    if($data['coupon_id']){
        //优惠券收益 资金计算
        $coupon_info = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."user_coupon where id = '".$data['coupon_id']."'");
        $deal = get_deal($deal['id']);
        //判断是否了优惠券
        $used_coupon_info = get_coupon_money($deal,$coupon_info,$data['money']);
        if($used_coupon_info['coupon_interests']){
           $pre_income += $used_coupon_info['coupon_interests'];
        }
    }

    $res = substr(sprintf("%.3f", $pre_income), 0, -1);
    return $res;
}
