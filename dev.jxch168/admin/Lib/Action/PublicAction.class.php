...
class PublicAction extends BaseAction{
	...
	//保存企创日志
        public function save_qichuang(){
            $orderno = $_REQUEST['orderno'];
            $phone = $_REQUEST['phone'];
            if($phone && $orderno){
              file_put_contents(APP_ROOT_PATH . 'log/qichuang/' . date('Y-m-d') . '_qichuang_orderno.log', "data:[手机号：" . $phone . ",订单号：".$orderno."];[" . date('Y-m-d H:i:s') . "]\r\n", FILE_APPEND);
            }
            $this->ajaxReturn('','企创订单保存成功！', 1);
        }
}
?>