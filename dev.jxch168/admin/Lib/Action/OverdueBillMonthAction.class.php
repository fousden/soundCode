...
class OverdueBillMonthAction extends CommonAction{
	...
	public function all_loan()
	{
		...
		if($rs_count > 0){
			...
			$sql_list =  " SELECT d.*,a.adm_name as admin_name,c.name as customer_name FROM ".DB_PREFIX."deal d left join ".DB_PREFIX."admin a on d.admin_id = a.id left join ".DB_PREFIX."customer c on c.id = d.id  WHERE $condition ORDER BY $order $sort LIMIT ".$p->firstRow . ',' . $p->listRows;
