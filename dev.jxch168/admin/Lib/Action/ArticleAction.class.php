...
class ArticleAction extends CommonAction{
    public function index(){
                ...
                if(trim($_REQUEST['cate_id'])!='')
		{
                    ...
                    $cate_id_son = M('ArticleCate')->field('id')->where(array('pid'=>$cate_id))->select();
                    $cate_id_son = array_map('array_shift', $cate_id_son);
                    if($cate_id_son){
                        array_push($cate_id_son,$cate_id);
                    }else{
                        $cate_id_son = $cate_id;
                    }
                    $condition['cate_id'] = array('in',$cate_id_son);			
		}
                if($_REQUEST['is_effect']!=''){
                    switch($_REQUEST['is_effect']){
                        case 1:
