...
class DealAction extends CommonAction {
    ...
    public function index() {
        ...
        if (trim($_REQUEST['deal_id']) != '') {
            $map['id'] = array('eq',intval($_REQUEST['deal_id']));
        }

        if(trim($_REQUEST['agency_name']) != ''){
            $agency_name = trim($_REQUEST['agency_name']);
            $agency_id_arr = M("user")->field("id")->where("user_name like '%".$agency_name."%'")->find();
            $agency_id = implode(",",$agency_id_arr);
            $map['agency_id'] = array('in','('.$agency_id.')');
        }
	...
	$data['end_time']=$data['start_time']+3600*24*$data['enddate'];//结束时间
	$data['qixi_time']=date("Y-m-d",$data['end_time']+3600*24*1);//起息时间
	$data['jiexi_time']=date("Y-m-d",  strtotime($data['qixi_time'])+3600*24*$data['repay_time']);//结息时间
	$data['last_mback_time']=date("Y-m-d",strtotime($data['jiexi_time'])+3600*24*2);//默认是结息日后3天
        // 更新数据
...
//        echo $asc;
    ...
    public function insert() {
        ...
        $data = M(MODULE_NAME)->create();
        if ($_REQUEST['icon']) {
            $deal = M("deal")->query("select max(id) from fanwe_deal");
            $deal_id = intval($deal[0]['max(id)'] + 1);
            foreach ($_REQUEST['icon'] as $v) {
                if (!empty($v)) {
                    $icon_url['icon_url'] = $v;
                    $icon_url['deal_id'] = $deal_id;
                    M('deal_gallery')->add($icon_url);
                }
            }
        }
        //开始验证有效性
        ...
        if ($data['yield_ratio'] <= 0) {
            $this->error(L("DEAL_YIELD_REBATE_TIP"));
        }
	...
	$data['qixi_time']=date("Y-m-d",$data['end_time']+3600*24*1);//起息时间
	$data['jiexi_time']=date("Y-m-d",  strtotime($data['qixi_time'])+3600*24*$data['repay_time']);//结息时间
	$data['last_mback_time']=date("Y-m-d",strtotime($data['jiexi_time'])+3600*24*2);//默认是结息日后3天内
    ...
    public function edit() {
        ...
        $icons = M("deal_gallery")->where("deal_id=$id")->field("icon_url")->select();
        $vo['start_time'] = $vo['start_time'] != 0 ? to_date($vo['start_time']) : '';
        $vo['jiexi_time'] = $vo['jiexi_time'] != 0 ? ($vo['jiexi_time']) : '';
        $vo['last_mback_time'] = $vo['last_mback_time'] != 0 ? ($vo['last_mback_time']) : '';
    ...
    public function update() {
        ...
        //如果满标不允许修改
        $deal_status = D("Deal")->where(array("id" => $data['id']))->getField("deal_status");
        if ($deal_status >= 2) {
            $this->error("该标的已招标成功，暂不支持修改，如需修改请联系系统管理员！");
        }

        if($_REQUEST['icon']){
            M('deal_gallery')->where("deal_id={$data['id']}")->delete();
            foreach ($_REQUEST['icon'] as $key=>$val){
                if(!empty($val)){
                $deal_icon['icon_url']=$val;
                $deal_icon['deal_id']=$data['id'];
                M('deal_gallery')->where("deal_id={$data['id']}")->add($deal_icon);
                }
            }
        }
        $log_info = M(MODULE_NAME)->where("id=" . intval($data['id']))->getField("name");
        ...
        $data['benxibaozhang'] = trim($data['benxibaozhang']);
        //成功时间
        $success_time = M("deal")->where(array("id"=>$data['id']))->getField("success_time");
        //标的开始时间
        $start_time = M("deal")->where(array("id"=>$data['id']))->getField("start_time");
        //更新标的起息时间 结息时间
        if($success_time){
            $data['qixi_time'] = date("Y-m-d", strtotime("+1 day", $success_time));
        }else{
            $data['end_time']=$start_time+3600*24*$data['enddate'];//结束时间
            $data['qixi_time']=date("Y-m-d",$data['end_time']+3600*24*1);//起息时间
        }    
        $data['jiexi_time'] = date("Y-m-d", strtotime("+" . $data['repay_time'] . " days", strtotime($data['qixi_time'])));
    ...
    public function foreverdelete() {
        ...
        if (isset($id)) {
            ...
            M("deal_gallery")->where(array('deal_id' => array('in', explode(',', $id))))->delete();
    ...
    public function set_moving() {
        $id = intval($_REQUEST['id']);
        $ajax = intval($_REQUEST['ajax']);
        $info = M(MODULE_NAME)->where("id=" . $id)->getField("name");
        $c_is_advance = M(MODULE_NAME)->where("id=" . $id)->getField("is_moving");  //当前状态
        $n_is_advance = $c_is_advance == 0 ? 1 : 0; //需设置的状态
        M(MODULE_NAME)->where("id=" . $id)->setField("is_moving", $n_is_advance);
    ...
    public function show_detail() {
        ...
        $count = D("DealLoad")->where(array('deal_id' => $id, 'is_auto' => 0,"contract_no"=>array("neq","")))->order("id ASC")->count();
        if (!empty($_REQUEST ['listRows'])) {
            $listRows = $_REQUEST ['listRows'];
        } else {
            $listRows = '';
        ...
        if ($count > 0) {
            $loan_list = D("DealLoad")->where(array('deal_id' => $id, 'is_auto' => 0,"contract_no"=>array("neq","")))->order("id ASC")->limit($p->firstRow . ',' . $p->listRows)->findall();
        ...
        $where['deal_id'] = $id;
        $where['is_auto'] = 1;
        $all_auto_money = M("deal_load")->field("sum(money) as all_money")->where($where)->select();
        $where['is_auto'] = 0;
        $all_hand_money = M("deal_load")->field("sum(money) as hand_money_all")->where($where)->select();

        //$adm_session = es_session::get(md5(conf("AUTH_KEY")));
	//$adm_id = intval($adm_session['adm_id']);
        //$admin_info = M('admin')->where(array('role_id'=>7))->find($adm_id);
        //$this->assign("oper_password", $admin_info['oper_password']);
        //$this->assign("adm_id", $adm_id);
        //$this->assign("role_id", $admin_info['role_id']);

        //实际筹款
        $deal_info['act_load_money'] = $all_hand_money[0]['hand_money_all'];
        //自动投标
        $deal_info['auto_load_money'] = $all_auto_money[0]["all_money"];
        //筹的款项
        $deal_info['load_moneys'] = $deal_info['act_load_money'] + $deal_info['auto_load_money'];
    ...
    public function repay_plan() {
        ...
        $deal_load_repay_list = $GLOBALS['db']->getRow("SELECT sum(repay_money) as total_repay_money,sum(interest_money) as total_interest_money FROM " . DB_PREFIX . "deal_load_repay where deal_id=" . $deal_info['id']);
...
//        $this->assign("role_id", $admin_info['role_id']);
        ...
        $this->assign("deal_load_repay_list", $deal_load_repay_list);
...
//        if($_REQUEST['oper_pwd']){
//            $oper_pwd = md5(trim($_REQUEST['oper_pwd']));
//            $adm_session = es_session::get(md5(conf("AUTH_KEY")));
//            $adm_id = intval($adm_session['adm_id']);
//            $admin_info = M('admin')->where(array('role_id'=>7))->find($adm_id);
//            if($oper_pwd != $admin_info['oper_password']){
//                $result['status'] = 0;
//                $result['info'] = "管理员操作密码输入错误，请重新输入！";
...
//        }
    ...
    function do_export_load($page = 1) {
        ...
        $model = new Model();
        $sql = "select dl.*,d.name,d.enddate,d.last_mback_time,d.rate,d.deal_status,u.real_name,d.yield_ratio,d.repay_time,d.qixi_time,d.jiexi_time from ".DB_PREFIX."deal_load dl left join ".DB_PREFIX."deal d on dl.deal_id = d.id left join ".DB_PREFIX."user u on dl.user_id = u.id where dl.is_auto = 0 AND dl.deal_id = ".$id;
        $deal_load_list = $model->query($sql);
        ...
        foreach($deal_load_lists as $key => $value){
            ...
            //获取活动收益 并加入到总收益中
            $mid_autumn_act = C('mid_autumn_acting');
            $active_interest_money = get_act_income($mid_autumn_act,$pre_income,$value['create_time']);
            if($active_interest_money){
                $active_interest_money = sprintf("%.2f", substr(sprintf("%.4f", $active_interest_money), 0, -2));
                $pre_income += $active_interest_money;
            }

            $deal_load_infos = $GLOBALS['db']->getRow("select coupon_id,coupon_interests from ".DB_PREFIX."deal_load where id = '".$value['id']."'");
            $coupon_type = $GLOBALS['db']->getOne("select coupon_type from ".DB_PREFIX."user_coupon where id = '".$deal_load_infos['coupon_id']."'");
            if($coupon_type == 1 && $deal_load_infos['coupon_interests'] > 0){
                $pre_income += $deal_load_infos['coupon_interests'];
            }
            ...
            if($key == 0){
                ...
                $objPHPExcel->setActiveSheetIndex(0)
                          ...
                          ->setCellValue('J'.$num, $value['qixi_time'])
    ...
    function do_repay_plan_export_load() {
        ...
        //输出投标列表
        $repay_list = get_deal_user_load_list($deal_info, 0, $l_key, -1, 0, 0, 1, "");
        //导出Excel还款计划表
        require_once APP_ROOT_PATH."public/PHPExcel/PHPExcel.php";
        $objPHPExcel = new PHPExcel();
        $deal_repay_lists = array();
        foreach($repay_list['item'] as $key => $value){
        ...
        foreach($deal_repay_lists as $key => $value){
            ...
            if($key == 0){
                ...
                $objPHPExcel->getActiveSheet()->setCellValueExplicit('I'.$num,$value['month_repay_money'],PHPExcel_Cell_DataType::TYPE_STRING);
        ...
        $filename = $deal_info['name'] . "第" . ($l_key + 1) . "期还款计划";
        php_export_excel($objPHPExcel,$filename);
