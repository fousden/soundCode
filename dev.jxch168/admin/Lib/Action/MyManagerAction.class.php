...
class MyManagerAction extends CommonAction
{
    ...
    public function index()
    {
        ...
        //线下门店可以‘增加、删除’用户部门成员，部门成员不能
        $is_mendian    = M('admin')->field('id,is_department,pid')->where("id = {$adm_id}")->find();
        $is_mendian_id = $is_mendian['is_department'];
        $this->assign('is_mendian_id', $is_mendian_id);
        //部门成员登录也能看到相同门店下其他成员的列表
        if ($is_mendian['pid'] != "0") {
            $list = M('admin')->where("pid = {$is_mendian['pid']}")->select();
    ...
    public function add()
    {
        ...
        $adminInfo = $GLOBALS['db']->getRow(" SELECT * FROM " . DB_PREFIX . "admin WHERE id= " . $adm_id);
        //查询部门列表
        if (1 == $adminInfo['is_department'])
        {
            $adm_sql  = " SELECT * FROM " . DB_PREFIX . "admin WHERE is_delete= 0 and is_effect=1 and is_department = 1 and id = " . $adm_id;
        } else {
        ...
        if (1 == $adminInfo['is_department'])
        {
            $list = M("Role")->where("name =  '门店销售'")->findAll();
        } else {
            $list = M("Role")->findAll();
        }
