...
//		echo $model->getLastSql();
		if ($count > 0) {
			...
			if(MODULE_NAME == "Feedback"){
				$voList = $model->where($map)->order("id desc,msg_id asc")->limit($p->firstRow . ',' . $p->listRows)->findAll();
			}else{
...
//			echo $model->getlastsql();
			...
			$sort = $sort == 'desc' ? 1 : 0; //排序方式
                                                     foreach($voList as $k=>$v){
                                                         ...
                                                         $start_times = date("H:i:s",$start_time);
                                                         $voList[$k]['start'] = $start_date." ".$start_times;
                                                         $end_times=date("H:i:s",$start_time-1);
                                                         $end_time=$start_time+3600*24*$v['enddate'];
                                                         $end_date = date("Y-m-d",$end_time);
                                                         $voList[$k]['end'] = $end_date." ".$end_times;
                                                     }

			//模板赋值显示
                                                    $start_time = $voList['start_time'];
	 ...
	 * @throws ThinkExecption
	 +----------------------------------------------------------
	 */
	function _Sql_list($model, $sql_str, $parameter='', $sortBy = '', $asc = false,$limit = true)
	{
		//排序字段 默认为主键名
		if (isset ( $_REQUEST ['_order'] )) {
			$order = $_REQUEST ['_order'];
