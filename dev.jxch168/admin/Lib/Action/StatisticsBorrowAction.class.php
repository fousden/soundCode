...
//		echo abs(to_timespan($map['end_time']) - to_timespan($map['start_time'])) / 86400 + 1;
	...
	public function tender_account_ratio(){
		...
		if ($start_time == $end_time) {
			...
			$lists           = $this->get_tender_account($start_time);
//			//将数组重整，此数据是放到数据中的
...
//			}
			...
			foreach($lists as $val){
				$client_list[0][] = empty($val) ? 0 : $val ;
			}
			array_pop($client_list[0]);
			$client_list[0][7] = $start_time;
			$client_list[0][8] = $count;
			array_pop($lists);
			if (!empty($lists)) {
				foreach ($lists as $key => $val) {
					$list[] = round($val/$count*100,2);
				}
			}
			$st_list = $list;
			$client_side_list = $list;
			$date_list[]   = $start_time_int;
		}else{
			$this->assign('type', 'line'); //这个数据是告诉模块该显示哪个图标line表示条形图，pie表示饼图
			$date_lists = M("deal_load")->order("create_date asc")->where("create_date>=$start_time_int and create_date<=$end_time_int")->field("create_date")->group("create_date")->findAll();;
			//时间重新组装，去掉关联索引改成自然索引
			$date_list = array();
			if (!empty($date_lists)) {
				foreach ($date_lists as $k => $v) {
					$date_list[] = $v['create_date'];
				}
			}
			foreach($date_lists as $val){
				$sql_str = "select
					...
					count(*) as count
					from ".DB_PREFIX."deal_load where is_has_loans = 1 and create_date= '".$val['create_date']."'";
				$list= D()->query($sql_str);
				$clist[$val['create_date']][0] = $lists[0][0] = empty($list[0]['5千以下']) ? 0 : $list[0]['5千以下'];
				$clist[$val['create_date']][1] = $lists[0][1] = empty($list[0]['5千至1万']) ? 0 : $list[0]['5千至1万'];
				$clist[$val['create_date']][2] = $lists[0][2] = empty($list[0]['1至5万']) ? 0 : $list[0]['1至5万'];
				$clist[$val['create_date']][3] = $lists[0][3] = empty($list[0]['5至10万']) ? 0 : $list[0]['5至10万'];
				$clist[$val['create_date']][4] = $lists[0][4] = empty($list[0]['10至20万']) ? 0 : $list[0]['10至20万'];
				$clist[$val['create_date']][5] = $lists[0][5] = empty($list[0]['20至50万']) ? 0 : $list[0]['20至50万'];
				$clist[$val['create_date']][6] = $lists[0][6] = empty($list[0]['50万以上']) ? 0 : $list[0]['50万以上'];
				$lists[0][7] = $val['create_date'];
				$lists[0][8] = $list[0]['count'];
				$chart_lists[$val['create_date']] = $clist[$val['create_date']];
				$client_list[] = $lists['0'];
			}
		}
		$st_list = array();
		foreach($chart_lists as $key=>$val){
			$st_list[0][] = (int)$val[0];
			$st_list[1][] = (int)$val[1];
			$st_list[2][] = (int)$val[2];
			$st_list[3][] = (int)$val[3];
...
//		echo "<pre>";
//		print_r($st_list);exit;
		$data_name_raw = [ '5千以下', '5千至1万', '1至5万', '5至10万','10至20万','20至50万','50万以上'];
//		$data_name_raw = [ '5千以下'];
		...
		//传入的数据数组，必填，类型为数组
		$this->assign('data_array', json_encode(array_values($st_list)));
//		$st_list=[[22,22,232,343,4324,23423,4,23423,44,2132,323,323]];
//		$this->assign('data_array', json_encode($st_list));
//		echo '<pre>';var_dump($st_list);die;
	...
	function get_tender_account($date){
		if (!$date) {
		...
		sum(if(money >= 500000, 1, 0)) as 50万以上,
		count(*) as count
		from ".DB_PREFIX."deal_load where is_has_loans = 1 and create_date= '".$date."'";
		$info = D()->query($sql_str);
		return $info;
