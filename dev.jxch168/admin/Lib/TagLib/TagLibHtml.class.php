...
class TagLibHtml extends TagLib {//类定义开始
    ...
    public function _list($attr) {
        ...
        $class_key=array();
        $class_arr=array();
        foreach ($fields as $key => $field) {//显示指定的字段
            $property = explode('|', $field[0]);
            $showname = explode('|', $field[1]);
            if(stripos(trim($showname[1]),'class')!==false || stripos(trim($showname[2]),'class')!==false || stripos(trim($showname[3]),'class')!==false){
                $class_key[$key]=$key;
                $class_arr[$key]=trim($showname[1]);
            }
            if (isset($showname[1])) {
        ...
        foreach ($fields as $key => $field) {
            if(in_array($key,$class_key)){
                $parseStr .= '<td '.$class_arr[$key].'>&nbsp;';
            }else{
                $parseStr .= '<td>&nbsp;';
            }
            //显示定义的列表字段
    ...
    <input type='hidden' value='" . $svalue . "' name='" . $name . $imgArrFlag . "' id='keimg_h_" . $name . "_i' />
