...
var lottery = {
	...
	// 开启抽奖
	_enable: function () {
		this.container.find('.getbtn').bind('click', this.beforeRoll);
	    
	},

	// 禁用抽奖
	_disable: function () {
		this.container.find('.getbtn').unbind('click', this.beforeRoll);
	},
	...
	beforeRoll: function () {
                ...
                query.pwd = user_info.pwd;
                $.ajax({
                    url: '/index.php?ctl=activity&act=lottery&type=market_activities&action=do_lottery&is_pc=1',
                    data: query,
                    type: "POST",
                    dataType: "json",
