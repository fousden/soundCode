$(document).ready(function(){
	...
	$('#shareBtn').click(function(){
		$('.alertcover,#alertshare').fadeIn(100);
	})
		...
		$.ajax({
			...
			success:function(data){
				if(data.code==-1){
					...
					window.open("./member.php?ctl=uc_bonus&act=index&type=1&is_pc=1")
				}else if(data.code==0){
					alert("领取成功，注册登录之后可在我的红包中查看");
					if(data.is_mobile==1){
						location.href="./wap/index.php?ctl=register&is_pc=1&channel="+qudao;
					}else{
					...
					if(data.is_mobile==1){
						location.href="./wap/index.php?ctl=login&_m=ios&is_pc=1";
					}else{
