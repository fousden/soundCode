	...
	public function index(){
		if($this->isPost()){
			...
			if($_GET['department_id']){  //部门下所有员工列表
				...
				//$user_list = $d_role->where('position.department_id = %d and status = 1', $department_id)->field('user_name,role_id,telephone,department_id,department_name,role_name,img')->select();	
				//$position_list = M('Position')->field('position_id')->select();
