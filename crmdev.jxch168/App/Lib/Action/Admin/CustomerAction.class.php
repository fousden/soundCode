...
class CustomerAction extends Action
{
    ...
    public function search()
    {
        ...
        if ($this->_get('create_time')) {
            ...
            switch ($create_times['condition']) {
                case "tgt" : $where['create_time'] = array('egt', $create_time);
                    break; //晚于
                case "lt" : $where['create_time'] = array('elt', $create_time);
                    break;  //早于
                case "between" : $where['create_time'] = array('between', array($create_time - 1, $create_time + 86400));
                    break; //在
                case "nbetween" : $where['create_time'] = array('not between', array($create_time, $create_time + 86399));
                    break; //不在
            }
        }
        ...
        if ($this->_get('update_time')) {
            ...
            switch ($update_times['condition']) {
                case "tgt" : $where['update_time'] = array('egt', $update_time);
                    break; //晚于
                case "lt" : $where['update_time'] = array('elt', $update_time);
                    break;  //早于
                case "between" : $where['update_time'] = array('between', array($update_time - 1, $update_time + 86400));
                    break; //在
                case "nbetween" : $where['update_time'] = array('not between', array($update_time, $update_time + 86399));
                    break; //不在
            }
        }
            ...
            if ($m_customer->create()) {
                if ($m_customer_data->create() !== false) {
                    if ($_POST['con_name']) {
                        $contacts                  = array();
                        if ($_POST['con_name'])
                            $contacts['name']          = $_POST['con_name'];
                        if ($_POST['owner_role_id'])
                            $contacts['owner_role_id'] = $_POST['owner_role_id'];
                        if ($_POST['saltname'])
                            $contacts['saltname']      = $_POST['saltname'];
                        if ($_POST['con_email'])
                            $contacts['email']         = $_POST['con_email'];
                        if ($_POST['con_post'])
                            $contacts['post']          = $_POST['con_post'];
                        if ($_POST['con_qq'])
                            $contacts['qq']            = $_POST['con_qq'];
                        if ($_POST['con_telephone'])
                            $contacts['telephone']     = $_POST['con_telephone'];
                        if ($_POST['con_description'])
                            $contacts['description']   = $_POST['con_description'];
                        if (!empty($contacts)) {
    ...
    public function index()
    {
        ...
        switch ($by) {
            ...
            case 'me' : $where['owner_role_id'] = session('role_id');
                break;
            case 'focus' : $where['customer_id']   = array('in', $focus_id);
                break;
        ...
        if (trim($_GET['act'] == 'sms')) {
            ...
            foreach($list as $key=>$val){
                $list[$key]["customer_invest_money"] = (M("contract")->where(array("customer_id"=>$val['customer_id']))->getField("sum(investment_money)")) / 100;
           }

            // var_dump($list,$d_v_customer);exit;
            $count = $d_v_customer->where($where)->count();
            import("@.ORG.Page");
    ...
    public function view()
    {
        ...
        if (0 == $customer_id) {
            ...
            $customer["customer_invest_money"] = (M("contract")->where(array("customer_id"=>$customer['customer_id']))->getField("sum(investment_money)")) / 100;
    ...
    public function cares()
    {
        ...
        switch ($by) {
            ...
            case 'month' :
                ...
                break;
            case 'email' : $where['type']          = 'email';
                break;
            case 'phone' : $where['type']          = 'phone';
                break;
            case 'add' : $order                  = 'create_time desc';
                break;
            case 'update' : $order                  = 'update_time desc';
                break;
            case 'message' : $where['type']          = 'message';
                break;
            case 'other' : $where['type']          = 'other';
                break;
            default : $where['owner_role_id'] = array('in', implode(',', $all_ids));
                break;
        }
        if ($by != 'deleted') {
            $where['is_deleted'] = array('neq', 1);
    ...
    public function caresAdd()
    {
        ...
        if ($this->isPost()) {
            ...
            if ($m_cares->create()) {
                if (!$_POST['subject'])
                    $this->error(L('CARE_SUBJECT_CANNOT_BE_EMPTY!'));
                if (!$_POST['care_time'])
                    $this->error(L('CARE_CARETIME_CANNOT_BE_EMPTY'));
                if ($_POST['care_time'])
                    $m_cares->care_time       = strtotime($_POST['care_time']);
    ...
    public function caresEdit()
    {
        ...
        if ($care_id && !check_permission($care_id, 'customerCares'))
            $this->error(L('HAVE NOT PRIVILEGES'));
        ...
        if ($this->isPost()) {
            ...
            if ($m_cares->create()) {
                if (!$_POST['subject'])
                    alert('error', L('CARE_SUBJECT_CANNOT_BE_EMPTY'), $_SERVER['HTTP_REFERER']);
                if ($_POST['care_time'])
                    $m_cares->care_time   = strtotime($_POST['care_time']);
                $m_cares->update_time = time();
                if ($m_cares->save()) {
