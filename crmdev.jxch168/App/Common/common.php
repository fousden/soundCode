<?php
require_once APP_PATH.'Common/extend_common.php';
function deldir($dir) {
    $dh = opendir($dir);
...
function field($search, $condition = '') {
    switch ($condition) {
        ...
        case "neq" : $where = array('neq', $search);
            break;
        case "between" : $where = array('between', array($search - 1, $search + 86400));
            break;
        case "nbetween" : $where = array('not between', array($search, $search + 86399));
            break;
        case "tgt" : $where = array('gt', $search + 86400);
            break;
...
function actionLog($id, $param_name = '', $text = '') {
    ...
    if ($actionLog->add()) {
        ...
        if (strtolower(ACTION_NAME) == 'delete' || strtolower(ACTION_NAME) == 'completedelete' || strtolower(ACTION_NAME) == 'log_delete') {
...
function bsendemail($address, $title, $content, $file = array(), $author = false, $selfsmtp = false) {
    ...
    if ($author === true)
        ...
        $author = C('defaultinfo.name') . "-wukong";
            ...
            switch ($v['form_type']) {
                ...
                case 'editor' :
                    /* $upload_url = U('file/editor');
                      ...
                      <textarea name="'.$v['field'].'" id="'.$v['field'].'" style="width: 800px; height: 350px;">'.$value.'</textarea> &nbsp;  <span id="'.$v['field'].'Tip" style="color:red;"></span>';
                      break; */
                    if ($type == 'edit') {
...
//
//
//	preg_match('/<string xmlns=\"http:\/\/http://61.130.7.220:8023/MWGate/wmgw.asmx\/\">(.*)<\/string>/',$line,$str);
...
//		}
//	}
//	preg_match('/<string xmlns=\"http://61.130.7.220:8023/MWGate/wmgw.asmx/MongateMULTIXSend\/\">(.*)<\/string>/',$line,$str);
//	$result=explode("-",$str[1]);
//	if(count($result)>1){
//		//echo '发送失败返回值为:'.$line."请查看webservice返回值";
//		return $line;
...
//		$line=str_replace("<string xmlns=\"http://61.130.7.220:8023/MWGate/wmgw.asmx/MongateMULTIXSend/\">","",$line);
...
function check_permission($module_id, $module, $permission_field = 'owner_role_id') {
    ...
    $position_id = M("role")->where(array("role_id"=>$role_id))->getField("position_id");
    ...
    if($position_id == 57 || $position_id == 58 || $position_name='前台'){
        return true;
    }
...
function getCheckUrlByAction($m, $a) {
    switch (strtolower($a)) {
        ...
        case "getwarehouselist" : $a = 'warehouse';
            break;
        case "salesreturn" : $a = 'index';
            break;
        case "addsalesreturn" : $a = 'add';
            break;
        case "editsalesreturn" : $a = 'edit';
            break;
...
function getTags($limit) {
    $m_tags = M('tags');
    if (!empty($limit)) {
        $tags = $m_tags->limit($limit)->order('hits desc')->select();
    } else {
        $tags = $m_tags->order('hits desc')->select();
    }
    return $tags;
}
 ...
 * param 	: limit
  ...
  type = 2 推送多个设备
  model_type = 1 IOS
  model_type = 2 Android
 * */
/*  function Xinge($token,$title,$content,$type,$model_type){
  $access_id = '';
  $secret_key = '';
  import('@.ORG.XingeApp');
  if($model_type == 1){
  ...
  $result = XingeApp::PushTokenIos($access_id,$secret_key,$content,$token,XingeApp::IOSENV_DEV);
  }else{
  ...
  //推送消息给大批量设备，数量有限（1000个）(可多次)
  foreach($token as $v){
  if($v != ''){
  $tokenlist[] = $v;
  }
  }
  $token_list = array_chunk($tokenlist, 1000); //拆分数组
  foreach($token_list as $k=>$v){
  $push = new XingeApp($access_id,$secret_key);
  $result = $push->PushDeviceListMultiple($pushId,$v);
  }
  }
  }
  }elseif($model_type == 2){
  if($type == 1){
  XingeApp::PushTokenAndroid($access_id,$secret_key,$title,$content,$token);
  }else{
  $push = new XingeApp($access_id,$secret_key);
  $mess = new Message(); //$mess = new MessageIOS();
  //完善Message消息
 ...
 * author : xiaoxu
 * function : app列表权限返回
 * param 	: $m=MODULE_NAME $role_id 岗位ID $type=1 有role_id判断 $type=2 没有role_id 判断
 * */
 ...
 * @return type
 */
function e2Tonumber($str) {
    return $str / 100;
}

function productName($productId) {
    $productInfo = D('Product')->where("product_id = '" . $productId . "'")->select();
    return $productInfo[0]['name'];
}

//格式化处理合同

function format_contract_info($contract_info){
    foreach($contract_info as $key=>$val){
        if($key == "investment_money"){
            $contract_info["investment_money"] = $val / 100;
        }
        if($key == "month_investment_rate"){
            $contract_info["month_investment_rate"] = $val / 100;
        }
        if($key == "investment_rate"){
            $contract_info["investment_rate"] = $val / 100;
        }
        if($key == "active_rate"){
            $contract_info["active_rate"] = $val / 100;
        }
        if($key == "month_interest"){
            $contract_info["month_interest"] = $val / 100;
        }
        if($key == "total_interest"){
            $contract_info["total_interest"] = $val / 100;
        }
        if($key == "total_receivables_money"){
            $contract_info["total_receivables_money"] = $val / 100;
        }
    }
...
function get_file_size($file_path){
    return filesize($file_path) ? round((filesize($file_path)/1024),2)." KB" : "0 KB";
}
